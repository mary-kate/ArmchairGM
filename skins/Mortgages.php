...
class SkinMortgages extends Skin {
	
  #set stylesheet
  function getStylesheet() {
    return "common/vcu.css";
  }
  ...
  $s .= '<div id="container">';
	...
	$s .= '<div id="top">';
          $s .= '<div id="logo">';
            $s .= '<a href="index.php?title=Main_Page"><img src="images/vcu/logo.png" alt="logo" border="0"/></a>';
          $s .= '</div>';
          $s .= '<div id="toplinks">';
	...
	$s .= '<div id="navbar">';
          $s .= '<div id="leftNavLeft">';
            $s .= '<img src="images/vcu/leftNavLeft.png" alt="logo" border="0"/>';
          $s .= '</div>';
          $s .= '<div id="leftNavMiddle">';
            $s .= '<ul>';
	      $s .= '<li><a href="index.php?title=Campus_Life">Campus Life</a></li>';
	      $s .= '<li><a href="index.php?title=Category:Professors">Rate Your Professor</a></li>';
	      $s .= '<li><a href="index.php?title=Encylopedia">Encylopedia</a></li>';
	      $s .= '<li><a href="index.php?title=Things_To_Do_Around_VCU">Off Campus</a></li>';
            $s .= '<ul>';
         $s .= '</div>';
         $s .= '<div id="leftNavRight">';
           $s .= '<img src="images/vcu/leftNavRight.png" alt="logo" border="0"/>';
         $s .= '</div>';
         $s .= '<div id="rightNav">';
	    $s .= '<img src="images/armchairgmnew/pencilnav.gif" alt="logo" border="0"/> <a href="index.php?title=Create">write about vcu</a>';
          $s .= '</div>';
	  $s .= '<div class="cleared"></div>';
        $s .= '</div>';  #end navbar
	...
	if ($wgOut->getPageTitle() !== 'Main Page') {
	  ...
	  $s .= '<div id="userside">';
	      ...
	      $s .= '</p>';
	      $s .= '<p><img src="images/armchairgmnew/userpageIcon.png" alt="userpage icon" border="0"/> '.$this->makeKnownLinkObj( $wgUser->getUserPage(), "your userpage").'</p>';
	      $s .= '<p>';
	      $s .= '<img src="images/armchairgmnew/emailIcon.png" alt="email icon" border="0"/> ';
	  ...
	  if ( $wgOut->isArticle() ) {
	    ...
	    $s .= '<div id="middleside">';
	      $s .= '<p><img src="images/armchairgmnew/pagehistoryIcon.png" alt="page history icon" border="0"/> ' . $this->historyLink() . '</p>';
	      if ( $wgTitle->userCanMove() ) {
	        $s .= '<p><img src="images/armchairgmnew/moveIcon.png" alt="move icon" border="0"/> ' . $this->moveThisPage() . '</p>';
	      }
	      $s .= '<p><img src="images/armchairgmnew/whatlinkshereIcon.png" alt="what links here icon" border="0"/> ' . $this->whatLinksHere() . '</p>';
              if ( $wgUser->isAllowed('protect') && NS_SPECIAL !== $wgTitle->getNamespace() ) {
                $s .= '<p><img src="images/armchairgmnew/protectIcon.png" alt="protect icon" border="0"/> ' . $this->protectThisPage() . '</p>';
              }
	      if ( $wgUser->isAllowed('delete') && NS_SPECIAL !== $wgTitle->getNamespace()) {
		$s .= '<p><img src="images/armchairgmnew/deleteIcon.png" alt="delete icon" border="0"/> ' . $this->deleteThisPage() . '</p>';
	      }
	    $s .= '</div>';
	    $s .= '<div id="bottomside"></div>';
	  }
	  ...
	  if ( $wgUser->isLoggedIn() ) {
	    ...
	    $s .= '<div id="middleside">';
	      ...
	      $s .= '<p><a href="http://digg.com/submit?phase=2&url='.$wgTitle->getFullURL().'&title='.$wgTitle->getText().'"><img src="images/armchairgmnew/diggIcon.png" alt="digg icon" border="0"/> digg</a></p>';
	      $s .= '<p><a href="http://del.icio.us/post"><img src="images/armchairgmnew/deliciousIcon.png" alt="delicious icon" border="0"/> delicious</a></p>';
	    $s .= '</div>';
	    $s .= '<div id="bottomside"></div>';
      ...
      if ($tns == NS_USER || $tns == NS_USER_TALK) {
		...
		$s .= "<img src='images/avatars/" . $avatar->getAvatarImage() . "'/>";
	      ...
	      $s .= '<div id="userpageTop">';
	        $s .= $this->pageTitle();
		$s .= '<span class="edit"><a href="#"><img src="images/armchairgmnew/editicon.png" alt="logo" border="0"/> ' . $this->editThisPage() . '</a></span>';
 ...
 function doAfterContent() {
  ...
  if ($wgOut->getPageTitle() !== 'Main Page') {
	 ...
	 $s .= '<a href="index.php?title=Help">help</a>';
       ...
       $s .= '</p>';
       $s .= '<p>';
         $s .= '<a href="http://www.gnu.org/copyleft/fdl.html"><img src="images/journal/gnu-fdl.png" alt="GNU-FDL" border="0"/></a>';
         $s .= '<a href="http://www.mediawiki.org"><img src="images/journal/poweredby_mediawiki_88x31.png" alt="powered by mediawiki" border="0"/></a>';
       $s .= '</p>';
     $s .= '</div>';
 ...
 function getMainPage(){
  ...
  $output .= '<h1>what\'s vcu.wikia?</h1>';
  $output .= '<p><b>VCU.Wikia</b> is a user powered college community.  Our community writes articles about VCU, rates and reviews VCU professors, contributes to our growing school encyclopedia
  encylopedia, and talks about issues important to them.  Join the conversation.</p>';
  ...
  $output .= '<p><img src="images/armchairgmnew/userpageIcon.png" alt="userpage icon" border="0"/> '.$this->makeKnownLinkObj( $wgUser->getUserPage(), "your userpage").'</p>';
  $output .= '<p><img src="images/armchairgmnew/emailIcon.png" alt="email icon" border="0"/> ';
    if ( $wgUser->getNewtalk() ) {
  ...
  $output .= '<p><a href="index.php?title=Create"><img src="images/vcu/createicon.png" alt="icon" border="0"/> write an article</a></p>';
  $output .= '<p><a href="index.php?title=Encylopedia"><img src="images/vcu/encyclopediaicon.png" alt="icon" border="0"/> add to the vcu encyclopedia</a></p>';
  $output .= '</div>';
  ...
  return $output;
   ...
   function searchForm( $label = "" ) {
    ...
    $s .= "<input class='button' type='image' src='../images/journal/search.gif' value=\"" . htmlspecialchars( wfMsg( "go" ) ) . "\" />";
    $s .= "</div>";
