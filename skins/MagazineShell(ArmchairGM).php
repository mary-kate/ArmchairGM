 ...
 class SkinMagazineShell extends Skin {
	...
	*/
		  ...
		  $s .= '<div class="avatar-messages">';
		    ...
		    if($wgChallengesEnabled){
		      ...
		      if ( $challenge > 0 ) {
			      ...
			      $s .= '<span class="profile-on">' . "<a href=index.php?title=Special:ChallengeHistory&user=" . $title1->getDbKey() . "&status=0>New Challenge</a></span>";
			      $s .= '</p>';
		      }
		    }
	...
	function getMainPageUserBox($avatar){
		...
		$output = "";
		  		$output .= "<div class=\"profile-box-top-links\">
					<a href=\"index.php?title=User:{$wgUser->getName()}\">Profile</a> or <a href=\"index.php?title=Special:UserHome\">Friend's Activity</a>
				</div>";
				...
				$output .= "</div>";
					...
					if ($complete != 100) {
						...
						$output .= "<div class=\"profile-completeness\">";
							...
							$output .= "<div class=\"profile-complete-bar-container\">";
								$output .= "<div style=\"background-color:#89C46F;width:".($complete*2)."px; height:14px;\">&nbsp;</div>";
							$output .= "</div>";
					...
					if (($friend_request_count) || ($foe_request_count) || ($wgUser->getNewtalk()) 
					...
					$output .= "<div class=\"profile-box-messages\">";
						...
						if ( $wgUser->getNewtalk() ) {
							$output .= '<p>
								<img src="images/common/emailIcon.png" alt="email icon" border="0"/> 
								<span class="profile-on">' . 
									$this->makeKnownLinkObj($wgUser->getTalkPage(), "New Message") . '
								</span>
							</p>';
					    }
	...
	function getUserPageRelationships($user_name,$rel_type){
		...
		//try cache
		if(!$data){
			$friends = $rel->getRelationshipList($rel_type,4);
		...
		if($rel_type==1){
			$output .= "<h1 class=\"user-profile-title\">Friends ({$friend_count})</h1>
					<p class=\"profile-sub-links\">
						<a href=\"index.php?title=Special:ViewRelationships&user={$user_name}&rel_type={$rel_type}\">View All Friends</a>";
			...
			$output .= "<h1 class=\"user-profile-title\">Foes ({$foe_count})</h1>
					<p class=\"profile-sub-links\">
						<a href=\"index.php?title=Special:ViewRelationships&user={$user_name}&rel_type={$rel_type}\">View All Foes</a>";
						if($wgUser->getName() == $wgTitle->getText()) {
							$output .= " - <a href=\"index.php?title=Special:SimilarFans\">Meet Similar Fans</a>";
							$output .= " - <a href=\"index.php?title=Special:SimilarFans\">Add Fans</a>";
						}
	...
	function getUserPageActivity($user_name){
		...
		$output = "<script>
			...
			$output .= "<table border=\"0\" cellpadding=\"0\" cellspacing=\"0\" width=\"100%\">
				<tr>
					<td class=\"user-activity-title\">My Activity</td>
	...
	function getUserPageGifts($user_name){
		...
		if(1==1){//!$data){
			$user_gifts = $g->getUserGiftList(0,4);
		...
		$per_row = 4;
			...
			$output .= "<h1 class=\"user-profile-title\">Gifts and Awards (" . ($gift_count+$system_gift_count).")</h1>
					<p class=\"profile-sub-links\">
						<a href=\"index.php?title=Special:ViewGifts&user={$user_name}\">View All Gifts</a> or
						<a href=\"index.php?title=Special:ViewSystemGifts&user={$user_name}\">View All Awards</a>
					</p>";
	...
	function getUserProfile($user){
		...
		$output = "";			
		$output .= "<h1 class=\"user-profile-title\">More About {$wgTitle->getText()}</h1>";
				if ($wgUser->getName() == $wgTitle->getText()) {
	...
	function getUserStats($user_id,$user_name){
		...
		$output .= "<h1 class=\"user-profile-title\">My Stats</h1>
				<p class=\"profile-sub-links\" style=\"margin-bottom:15px\">
					<a href=\"index.php?title=Special:TopFansRecent&period=weekly\">This Week's Top Fans</a> or 
	...
	function getNewUsers($limit = 10){
		...
		$output .= '<div class="featured-users">';
			...
			if($wgUser->isLoggedIn() ){
				...
				$output .= "<p class=\"main-page-sub-links\"><a href=\"{$similar_title->getFullURL()}\">Meet Similar Fans</a> or <a href=\"{$fav_title->getFullURL()}&sport_id={$f[0]["sport_id"]}&team_id={$f[0]["team_id"]}\">Fans From Your Favorite Team</a></p>";
			}else{
				$output .= "<p class=\"main-page-sub-links\"><a href=\"{$register_title->getFullURL()}\">Meet Similar Fans</a> or <a href=\"{$register_title->getFullURL()}\">Fans From Your Favorite Team</a></p>";
			}
			$output .= $users;
		$output .= '</div>';
		return $output;
