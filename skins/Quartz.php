...
class QuartzTemplate extends QuickTemplate {
	...
	function getLoginLogoutUserMenu($left = true, $buttons = false) {
		...
		if(!$this->data['loggedin']) {
			...
			$o .= '<a href="' . htmlentities( htmlspecialchars( empty( $this->data['personal_urls']['login'] ) ? $this->data['personal_urls']['anonlogin']['href'] : $this->data['personal_urls']['login']['href'] ) ) . '" id="login">' . wfMsgExt('login', array('parseinline')) . '</a>';
			if($buttons == true) {
			...
			foreach($userMenuLinks as $linkkey => $linkval) {
				$o .= '<li><a id="um-'. $linkkey .'" href="' . htmlentities($linkval['href']) . '">' . $linkval['text'] . '</a></li>';
			}
			$o .= '</ul></div></div></li><li style="background: none;"> - <span>[</span><a id="logout" href="' . htmlspecialchars( $this->data['personal_urls']['logout']['href'] ) . '">' . htmlspecialchars( $this->data['personal_urls']['logout']['text'] ) . '</a><span>]</span></li>';
		}
		$o .= '</ul>';
		return $o;
	}
	...
	function getAds($pos) {
		...
		if(!empty($wgForceSkin) && $wgForceSkin == 'quartz-gamespot') {
			$wgNoWideAd = true;
		}
		switch( $pos ) {
			case ADSERVER_POS_TOPRIGHT :
				if($wgUser->isLoggedIn() || $wgNoWideAd) {

					$spots = '';

					if( !empty( $this->data['adserver_ads'][ADSERVER_POS_TOPRIGHT_USER] ) ) {
						$spots .= "\n\t".'<div id="spotlight-1">'.$this->data['adserver_ads'][ADSERVER_POS_TOPRIGHT_USER].'</div>'."\n";
					}
					if( !empty( $this->data['adserver_ads'][ADSERVER_POS_SPOTLIGHT_USER] ) ) {
						$spots .= "\n\t".'<div id="spotlight-2">'.$this->data['adserver_ads'][ADSERVER_POS_SPOTLIGHT_USER].'</div>'."\n";
					}

					// macbre: return spotlights container only if we have spotlights
					return (!empty($spots) ? '<div id="spotlights" class="clearfix">'.$spots.'</div>' : '<!-- no spotlights were defined -->');
				}
				else
				{
					return empty( $this->data['adserver_ads'][ADSERVER_POS_TOPRIGHT_ANON] ) ? null : $this->data['adserver_ads'][ADSERVER_POS_TOPRIGHT_ANON];
				}


			case ADSERVER_POS_BOTRIGHT :
				if($wgUser->isLoggedIn() || $wgNoWideAd) {
					return empty( $this->data['adserver_ads'][ADSERVER_POS_BOTRIGHT_USER] ) ? null : $this->data['adserver_ads'][ADSERVER_POS_BOTRIGHT_USER];
				} else {
					return empty( $this->data['adserver_ads'][ADSERVER_POS_BOTRIGHT_ANON] ) ? null : $this->data['adserver_ads'][ADSERVER_POS_BOTRIGHT_USER];
				}
			default :
				return empty( $this->data['adserver_ads'][$pos] ) ? null : $this->data['adserver_ads'][$pos];
		}
	}
	...
	function execute() {
		...
		global $wgShowAds, $wgUseAdServer, $wgNoWideAd;
...
exit();
		...
		$widgetManager = new WidgetManager( 'slate', &$this );

		// skip some Slate widgets
		$widgetManager->skip( "WidgetThisWiki" );
		$widgetManager->skip( "WidgetTopFive" );
		$widgetManager->skip( "WidgetSearch" );
		$widgetManager->skip( "WidgetThisArticle" );
		$widgetManager->skip( "WidgetExpertTools" );
		$widgetManager->skip( "WidgetWikiaMessages" );
		$widgetManager->skip( "WidgetLanguageUrls" );
...
<head>
	...
	<style type="text/css" media="screen,projection">/*<![CDATA[*/@import "<?php $this->text('stylepath') ?>/monobook/main.css?<?= $GLOBALS['wgStyleVersion'] ?>"; /*]]>*/</style>
...
<?php if($wgContLang->isRTL()) { ?>
	<link rel="stylesheet" href="<?php $this->text('stylepath') ?>/quartz/css/main_rtl.css?<?= $GLOBALS['wgStyleVersion'] ?>" type="text/css" />
<?php } ?>

	<!-- widgets -->
	<link rel="stylesheet" href="<?php $this->text('stylepath') ?>/common/widgets/css/widgets_base.css?<?= $GLOBALS['wgStyleVersion'] ?>" />
	<link rel="stylesheet" href="<?php $this->text('stylepath') ?>/common/widgets/css/widgets.css?<?= $GLOBALS['wgStyleVersion'] ?>" />
...
<body <?php if($this->data['body_onload']) { ?>onload="<?php $this->text('body_onload') ?>"<?php } ?> class="mediawiki <?php $this->text('nsclass') ?> <?php $this->text('dir') ?> <?php $this->text('pageclass') ?>">
		...
		<div class="r_boxContent" style="position: relative;">
			...
			<div id="mycarousel" class="carousel-component">
				<div class="carousel-clip-region" id="widget_cockpit_overlay" style="margin: 0">
					...
					<ul><li id="ghost" style="background-color: #cccccc;border:2px solid #000000;height: 35px;width: 35px;display: none; visibility: hidden;"></li></ul>
				</div>
			</div>
...
?>
	...
	<tbody>
		<tr>
			<td class="col1" width="55%">
				<a id="fe_edit_icon" href="<?= $wgTitle->getEditURL() ?>"><img src="<?php $this->text('stylepath') ?>/quartz/images/icon_edit.gif"/></a>
				<?= wfMsg('footer_1', $wgSitename) ?> <a id="fe_edit_link" href="<?= $wgTitle->getEditURL() ?>"><?= wfMsg('footer_1.5') ?></a><br/>
<?php
	if(!$wgTitle->isTalkPage()) {
		$talkPageTitle = $wgTitle->getTalkPage();
?>
				<a id="fe_talk_icon" href="<?= $talkPageTitle->getLocalURL() ?>"><img src="<?php $this->text('stylepath') ?>/quartz/images/icon_page_white_stack.png"/></a>
				<a id="fe_talk_link" <?= $talkPageTitle->exists() ? '' : ' class="new" ' ?>href="<?= $talkPageTitle->getLocalURL() ?>"><?= wfMsg('footer_2') ?></a><br/>
<?php
	}
...
?>
				<?= $userPageExists ? '<a id="fe_user_icon" href="'.$userPageLink.'">' : '' ?>
				<img src="<?php $this->text('stylepath') ?>/quartz/images/icon_user.png"/>
				<?= $userPageExists ? '</a>' : '' ?>
				<?= wfMsg('footer_5', '<a id="fe_user_link" '.($userPageExists ? '' : ' class="new" ').'href="'.$userPageLink.'">'.$userText.'</a>', $lastUpdate) ?>
<?php
	}
?>
				<div class="clearfix">
				    <h1 id="star-rating-header"><?= wfMsg('footer_9') ?></h1>
<?php
...
?>
			...
			<td class="col2">
				<a id="fe_random_icon" href="<?= Skin::makeSpecialUrl( 'Randompage' ) ?>"><img src="<?php $this->text('stylepath') ?>/quartz/images/icon_help.png"/></a>
				<a id="fe_random_link"href="<?= Skin::makeSpecialUrl( 'Randompage' ) ?>"><?= wfMsg('footer_6') ?></a>
				<br/>

				<a id="fe_create_icon" href="<?= Skin::makeSpecialUrl( 'Createpage' ) ?>"><img src="<?php $this->text('stylepath') ?>/quartz/images/icon_page_white_add.png"/></a>
				<a id="fe_create_link" href="<?= Skin::makeSpecialUrl( 'Createpage' ) ?>"><?= wfMsg('createpage') ?></a>
				<br/>
<?php
	if(!empty($wgNotificationEnableSend)) {
?>
				<a href="#"><img src="<?php $this->text('stylepath') ?>/quartz/images/icon_email.gif" id="shareEmail_img"/></a>
				<a href="#" id="shareEmail_a"><?= wfMsg('footer_7') ?></a>
				<br/>
<?php
	}
?>
				...
				<div id="shareIt">
					<span> <img alt="<?= wfMsg('footer_10', 'Delicious') ?>" id="shareDelicious_img" src="<?php $this->text('stylepath') ?>/quartz/images/icon_delicious.gif"/> <a href="#" id="shareDelicious_a"> del.icio.us</a></span>
					<span> <img alt="<?= wfMsg('footer_10', 'Stumbleupon') ?>" id="shareStumble_img" src="<?php $this->text('stylepath') ?>/quartz/images/icon_stumble.gif"/> <a href="#" id="shareStumble_a"> Stumbleupon</a></span>
					<span> <img alt="<?= wfMsg('footer_10', 'Digg') ?>" id="shareDigg_img" src="<?php $this->text('stylepath') ?>/quartz/images/icon_digg.gif"/> <a href="#" id="shareDigg_a"> Digg</a></span>
				</div>
			</td>
		</tr>
	</tbody>
...
<tr>
	...
	<div id="sidebar">
	<?php
		global $wgUseHelperPanel;
		if (isset($wgUseHelperPanel) && $wgUseHelperPanel) { ?>
		<noscript>
			<style type="text/css">
		...
		<style type="text/css">
			...
			{
				case 'helper':
					echo "#sidebar_widgets {display: none}";
					break;
				case 'widgets':
				default:
					echo "#sidebar_helper {display: none}";
					break;
			}
			?>
		</style>
		<div id="sidebar_buttons">
			<a href="#" id="sidebar_button_helper"><?= wfMsg('helperPanel_help') ?></a>
			<a href="#" id="sidebar_button_widgets"><?= wfMsg('helperPanel_widgets') ?></a>
		</div>
		<div id="sidebar_helper">
			<?php wfRunHooks('HelperPanelShow'); ?>
		</div>
	<?php } //$wgUseHelperPanel ?>
		<div id="sidebar_widgets">
			<!-- ads begin -->
			<div id="adSpace1" class="adSpace">
			<?= $this->getAds(ADSERVER_POS_TOPRIGHT) ?>
			</div>
			<!-- ads end -->

			<!-- widgets begin -->
			<ul class="widgets widgets_vertical" id="widgets_2">
			<?= $widgetManager->draw( 2 ); ?>
			</ul>
			<!-- widgets end -->

			<!-- ads begin -->
			<div id="adSpace2" class="adSpace">
			<?= $this->getAds(ADSERVER_POS_BOTRIGHT) ?>
			</div>
			<!-- ads end -->
		</div>
	</div>
...
<?php
	echo $this->getAds(ADSERVER_POS_JS_BOT1);
	echo $this->getAds(ADSERVER_POS_JS_BOT2);
	echo $this->getAds(ADSERVER_POS_JS_BOT3);
	echo $this->getAds(ADSERVER_POS_JS_BOT4);
	echo $this->getAds(ADSERVER_POS_JS_BOT5);

/**
 * eloy, display 1x1 dot for simple stats
 */
global $wgDotDisplay, $wgAdServerUrl, $wgAdServerTest;
if (!empty($wgDotDisplay)) {
    $wWebStats = new WikiaWebStats( $GLOBALS['wgCityId'], 0);
?>
    <img src="<?php echo $wgAdServerUrl; ?>/1dot.php?<?php echo (!empty($wgAdServerTest))?"db_test=1&":"" ?>js=<?php echo $wWebStats->makeStatUrl(); ?>" alt="." width="1" height="1" border="0" />
<?php
}
global $wgVisitorSkin, $wgVisitorTheme, $wgDefaultSkin, $wgDefaultTheme;
echo "<!-- temporary (by Inez) wgVisitorSkin: {$wgVisitorSkin} wgVisitorTheme: {$wgVisitorTheme} wgDefaultSkin: {$wgDefaultSkin} wgDefaultTheme: {$wgDefaultTheme} -->\n";
?>
<!-- analytics (end) -->
