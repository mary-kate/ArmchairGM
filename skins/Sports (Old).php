...
class SkinSports extends SkinMagazineShell {
  ...
  $s .= '<div id="container">';
	...
	$top_fans = Title::makeTitle(NS_SPECIAL,"TopFans");
		...
		$s .= "<div class=\"top-bar\">";
    		$s .= "<div class=\"logo\">";
		$s .= "<a href=\"{$main_page->escapeLocalURL()}\"><img src=\"{$wgUploadPath}/sports/logo.png\" alt=\"armchairgm\" border=\"0\"></a>";
    		$s .= "</div>";	
    		...
    		$s .= "<div class=\"add-friends\">";
    			if ($wgUser->isLoggedIn()) {
    				$s .= "<a href=\"{$invite->escapeLocalURL()}\" rel=\"nofollow\">+ Add Friends</a>";
    			} else {
    				$s .= "<a href=\"{$register->escapeLocalURL()}\" rel=\"nofollow\">+ Add Friends</a>";
    			}
    			
    		$s .= "</div>";
    			if ($wgUser->isLoggedIn()) {
					$s .= "<div class=\"other-links\">";
    				$s .= "<a href=\"{$site_scout->escapeLocalURL()}\" rel=\"nofollow\">Site Scout</a> - <a href=\"{$recent_changes->escapeLocalURL()}\"  rel=\"nofollow\">Recent Changes</a> - <a href=\"index.php?title=Special:TopFans\" rel=\"nofollow\">Top Fans</a> - ";
    				$s .= "<a href=\"{$logout->escapeLocalURL()}\" rel=\"nofollow\">Log Out</a>";
    			} else {
					$s .= "<div class=\"other-links-out\">";
    				$s .= "<span class=\"grey\">You are Not Logged-In</span><br>";
    				$s .= "<a href=\"{$register->escapeLocalURL()}\" rel=\"nofollow\">Sign Up</a> - <a href=\"{$login->escapeLocalURL()}\" rel=\"nofollow\">Log In</a>"; 
    			}
		...
		foreach($menu as $menu_item){
			if($menu_item["subpages"]){
				...
				foreach($menu_item["subpages"] as $subpage_display_name => $subpage_page_name){
					...
					if($x > 1)$s .= " - ";
					$s .= "<span class=\"{$sub_menu_class}\"><a href=\"{$menu_link}\" rel=\"nofollow\">{$subpage_display_name}</a></span>";
					$x++;
				}
				$s .= "</div>";
			}
    ...
    $s .= "<div class=\"main\">";
		if (   ($wgOut->getPageTitle() !== 'Main Page') && (NS_SPECIAL !== $wgTitle->getNamespace()) && (NS_USER !== $wgTitle->getNamespace()) && (NS_USER_TALK !== $wgTitle->getNamespace()) && (300 !== $wgTitle->getNamespace()) && ($wgTitle->getPrefixedText() !== "UserWiki:{$wgTitle->getText()}")) {
	  			...
	  			if (!($this->inCategory("Athletes"))) {
					...
					$s .= '<div id="main-body-full">';
					$s .= "<div class=\"edit-menu-full\">";
				}
	   				...
	   				$s .= "<div class=\"edit-button\" >";
						...
						if ( ! $wgOut->isArticleRelated() ) {
							...
							$s .= "<a href=\"{$wgTitle->getFullURL()}&action=edit\" rel=\"nofollow\">Locked</a>";
							$s .= "<span style=\"margin:0px 0px 0px 9px;\"><a href=\"javascript:editMenuToggle();\"  rel=\"nofollow\"><img src=\"images/common/edit-menu-arrow.gif\" alt=\"\" border=\"0\"/></a></span>";
						} else {
							if ( $wgTitle->userCan( 'edit' ) ) {
								$s .= "<img src=\"{$wgUploadPath}/common/editIcon.gif\"/>";
								$s .= "<a href=\"{$wgTitle->getFullURL()}&action=edit\" rel=\"nofollow\">Edit</a>";
								$s .= "<span style=\"margin:0px 0px 0px 29px;\"><a href=\"javascript:editMenuToggle();\" rel=\"nofollow\"><img src=\"images/common/edit-menu-arrow.gif\" alt=\"\" border=\"0\"/></a></span>";
							} else {
								$s .= "<img src=\"{$wgUploadPath}/common/editIcon.gif\"/>";
					...
					$s .= "<div class=\"edit-sub-menu\" id=\"edit-sub-menu-id\" style=\"display:none;\">";
						$s .= '<p><img src="'.$wgUploadPath.'/common/pagehistoryIcon.png" alt="" border="0"/> ' . $this->historyLink() . '</p>';
				   		if ( $wgTitle->userCanMove() ) {
					 		$s .= '<p><img src="'.$wgUploadPath.'/common/moveIcon.png" alt="" border="0"/> ' . $this->moveThisPage() . '</p>';
				       	}
				       	$s .= '<p><img src="images/common/whatlinkshereIcon.png" alt="" border="0"/> ' . $this->whatLinksHere() . '</p>';
				       	if ( $wgUser->isAllowed('protect') && NS_SPECIAL !== $wgTitle->getNamespace() && $wgTitle->exists() ) {
					 		$s .= '<p><img src="'.$wgUploadPath.'/common/protectIcon.png" alt="" border="0"/> ' . $this->protectThisPage() . '</p>';
				       	}
				       	if ( $wgUser->isAllowed('delete') && NS_SPECIAL !== $wgTitle->getNamespace() && $wgTitle->exists()) {
					 		$s .= '<p><img src="'.$wgUploadPath.'/common/deleteIcon.png" alt="" border="0"/> ' . $this->deleteThisPage() . '</p>';
				       	}
				       	if ( $wgUser->isLoggedIn() ) {
					   		$s .= '<p><img src="'.$wgUploadPath.'/common/addtowatchlistIcon.png" alt="" border="0"/> ' . $this->watchThisPage() . '</p>';
					 		$s .= '<p><img src="'.$wgUploadPath.'/common/uploadIcon.png" alt="" border="0"/> ' . $this->specialLink("upload") . '</p>';
				       }
	   				$s .= "</div>";
				$s .= "</div>";
	   			$s .= $this->pageTitle();
					...
					$s .= "<div class=\"user-profile-avatar-bar\">";
  						$s .= "<div class=\"user-profile-avatar\">";
							...
							$s .= "<p class=\"profile-box-points\">
								...
								<a href=\"{$level_link->getFullURL()}\" rel=\"nofollow\">({$user_level->getLevelName()})</a>
							</p>";
  				...
  				if ( $wgUser->getName() == $wgTitle->getText()  ) {
					...
					if ($complete != 100) {
						$s .= "<div class=\"profile-completeness\">";
							...
							$s .= "<div class=\"profile-complete-bar-number\">";
								$s .=  "<a href=\"index.php?title=Special:UpdateProfile\" rel=\"nofollow\">" . $complete . "% (edit)</a>";
							$s .= "</div>";
							$s .= '<div class="cleared"></div>';
						$s .= "</div>";
					}
		    		...
		    		$s .= '<p>
						<a href="index.php?title=Special:InviteContacts" rel="nofollow">
							<img src="../../images/common/msnIconSmall.gif" alt="" border="0"/> Hotmail
						</a>
						<a href="index.php?title=Special:InviteContacts" rel="nofollow">
							<img src="../../images/common/yahooIconSmall.gif" alt="" border="0"/> Yahoo
						</a>
						<a href="index.php?title=Special:InviteContacts" rel="nofollow">
							<img src="../../images/common/gmailIconSmall.gif" alt="" border="0"/> Gmail
						</a> 
						<a href="index.php?title=Special:InviteContacts" rel="nofollow">
							<img src="../../images/common/aolIconSmall.gif" alt="" border="0"/> AOL
						</a>
					</p>';
					$s .= "</div>";
		    	} else {
						...
						$s .= "<p>";
							if ($relationship==true) {
		      					...
		      					if ($relationship == 2) {
			    					...
			    					$s .= "<a href=\"index.php?title=Special:AddRelationship&user={$user_safe}&rel_type=1\" rel=\"nofollow\">
											<img src=\"../../images/common/friendRequestIcon.gif\"> Add as Friend
									</a> ";
			    					$s .= "<a href=\"index.php?title=Special:AddRelationship&user={$user_safe}&rel_type=2\" rel=\"nofollow\">
										   <img src=\"../../images/common/foeRequestIcon.gif\"> Add as Foe
									</a> ";
			    			}
							$s .= "<a href=\"index.php?title=Special:GiveGift&user={$user_safe}\" rel=\"nofollow\"><img src=\"../../images/common/icon_package_get.gif\"> Send A Gift</a> ";
  			...
  			$s .= "</div>";
  			  $s .= "<div class=\"user-gifts\">";
  				$s .= $this->getUserPageAwards($wgTitle->getText());
  			$s .= "</div>";
			...
			$s .= "<div class=\"user-picture-games\">";
				$s .= $this->getUserPolls($id, $wgTitle->getText());
			$s .= "</div>";

			$s .= "<div class=\"user-picture-games\">";
				$s .= $this->getUserQuiz($id, $wgTitle->getText(),$stats_data );
			$s .= "</div>";
  		...
  		$s .= "<div class=\"cleared\"></div>";
			...
			$s .= "<p><a href=\"index.php?title=User:{$wgTitle->getText()}\" rel=\"nofollow\"><< Back to {$wgTitle->getText()}'s Profile</a></p>";
  			$s .= "<div class=\"edit-menu-sub-profile\">";
   				$s .= "<div class=\"edit-button\" >";
					...
					if ( ! $wgOut->isArticleRelated() ) {
						...
						$s .= "<a href=\"{$wgTitle->getFullURL()}&action=edit\" rel=\"nofollow\">Locked</a>";
						$s .= "<span style=\"margin:0px 0px 0px 9px;\"><a href=\"javascript:editMenuToggle();\" rel=\"nofollow\"><img src=\"images/common/edit-menu-arrow.gif\"/></a></span>";
					} else {
						if ( $wgTitle->userCan( 'edit' ) ) {
							$s .= "<img src=\"images/common/editIcon.gif\"/>";
							$s .= "<a href=\"{$wgTitle->getFullURL()}&action=edit\" rel=\"nofollow\">Edit</a>";
							$s .= "<span style=\"margin:0px 0px 0px 29px;\"><a href=\"javascript:editMenuToggle();\" rel=\"nofollow\"><img src=\"images/common/edit-menu-arrow.gif\"/></a></span>";
						} else {
							$s .= "<img src=\"images/common/editIcon.gif\"/>";
							$s .= "<a href=\"{$wgTitle->getFullURL()}&action=edit\" rel=\"nofollow\">Locked</a>";
							$s .= "<span style=\"margin:0px 0px 0px 9px;\"><a href=\"javascript:editMenuToggle();\" rel=\"nofollow\"><img src=\"images/common/edit-menu-arrow.gif\"/></a></span>";
 ...
 function getMainPage(){
  ...
  if (!($wgUser->isLoggedIn())) {
	 ...
	 //WELCOME MESSAGE
		 ...
		 $good_count = number_format(SiteStats::articles());
			...
			$output .= '<div class="welcome-message">';
				...
				$output .= '<p class="welcome-big-link"><a href="index.php?title=Special:UserRegister" rel=\"nofollow\">Sign Up!</a></p>';
			$output .= '</div>';
		...
		//LOGGED OUT ARTICLES
		  ...
		  $output .= "<p class=\"main-page-sub-links\"><a href=\"index.php?title=Special:ArticlesHome\">Read More Articles</a> - <a href=\"index.php?title=Special:UserRegister\" rel=\"nofollow\">Write Your Own Article</a></p>";
		...
		foreach($ratings as $item) {		
			...
			$ratings_list .= "<div class=\"featured-rating\">
						<div class=\"featured-rating-title\">
							<a href=\"{$rating_title->getFullURL()}\" rel=\"nofollow\">{$rating_title->getText()}</a>
						</div>
	...
	$output .= $this->getFeaturedUsers();
			...
			foreach($fans as $fan) {		
				...
				$topfans .= "<div class=\"top-fan\"><span class=\"top-fan-number\">{$x}.</span> <a href=\"{$user->getFullURL()}\" rel=\"nofollow\">{$avatar_image}</a> <span class=\"top-fans-user\"><a href=\"{$user->getFullURL()}\" rel=\"nofollow\">{$fan["user_name"]}</a></span> <span class=\"top-fans-points\"><b>+" . number_format($fan["points"])."</b> points</span></div>";
				$x++;
			...
			$output .= '<div class="top-fans">';
				...
				$output .= "<p class=\"main-page-sub-links\"><a href=\"{$weekly_fans_title->getFullURL()}\" rel=\"nofollow\">This Week's Top Fans</a> - <a href=\"{$top_fans_title->getFullURL()}\" rel=\"nofollow\">Complete List</a></p>";
				$output .= $topfans;
			$output .= '</div>';
			...
			foreach($gifts as $gift) {		
				$recent_gifts .= "<div class=\"recent-gift\"><a href=\"{$gift_title->getFullURL()}&gift_id={$gift["id"]}\" rel=\"nofollow\"><img src=\"images/awards/" . Gifts::getGiftImage($gift["gift_id"],"l") . "\" border=\"0\" alt=\"\" /></a> </div>";
			}
		
			$output .= '<div class="recent-gifts">';
				$output .= '<h1>Receive Gifts and Awards</h1>';
				$output .= "<p class=\"main-page-sub-links\"><a href=\"index.php?title=Special:UserRegister\" rel=\"nofollow\">Give a Gift</a> - <a href=\"index.php?title=Special:UserRegister\" rel=\"nofollow\">Learn About Awards</a></p>";
				$output .= $recent_gifts;
				$output .= "<div class=\"cleared\"></div>";
			$output .= '</div>';		
  	...
  	$output .= "<div class=\"main-page-left\">";
		...
		$list = new ListPages();
		  ...
		  $output .= '<h1 >Popular Sports Articles <span class="rss-feed"><a href="http://feeds.feedburner.com/Armchairgm" rel="nofollow"><img src=http://www.armchairgm.com/images/a/a7/Rss-icon.gif border="0" alt=""></a> rss feed</span></h1>';
		  	$output .= "<p class=\"main-page-sub-links\"><a href=\"index.php?title=Create_Opinion\" rel=\"nofollow\">Write An Article</a> - <a href=\"index.php?title=Category:" . date("F j, Y") . "\">Today's Articles</a> - <a href=\"index.php?title=Special:ArticlesHome\">All Popular</a></p>";
		  	$output .= $list->DisplayList();
		  $output .= '</div>';
		  ...
		  $output .= "<p class=\"main-page-sub-links\" style=\"margin-bottom:10px;\"><a href=\"index.php?title=Create_Opinion\" rel=\"nofollow\">Write An Article</a> - <a href=\"index.php?title=Category:" . date("F j, Y") . "\">Today's Articles</a> - <a href=\"index.php?title=Special:ArticlesHome\">All New</a></p>";
		  $output .= $list->DisplayList();
		  $output .= '</div>';
	...
	$output .= "<div class=\"main-page-right\">";
			...
			*/
			...
			if($activity){
				...
				foreach ($activity as $item) {
					...
					if($user_title_2){
						$user_link_2 = "<a href=\"{$user_title_2->getFullURL()}\" rel=\"nofollow\">{$item["comment"]}</a>";
					}
					...
					$output .= "<span class=\"user-home-item-icon\"><img src=images/common/" . UserActivity::getTypeIcon($item["type"]) . " alt=\""  . UserActivity::getTypeIcon($item["type"]) . "\" border='0'></span><span class=\"user-home-item-user\"><a href=\"{$user_title->getFullURL()}\"  rel=\"nofollow\">{$item["username"]}</a></span><span>";
					switch ($item["type"]) {
						case "edit":
						...
						case "gift-rec":
							...
							$output .= "received a <a href=\"index.php?title=Special:ViewGift&gift_id={$item["id"]}\" rel=\"nofollow\">gift</a> from {$user_link_2} {$item_time}</span>";
							$output .= "<div class=\"user-home-item-gift\">";
							$output .= "<span class=\"user-home-gift-image\">";
							$output .= "<a href=\"index.php?title=Special:ViewGift&gift_id={$item["id"]}\" rel=\"nofollow\">{$gift_image}</a>";
							$output .= "</span>";
						...
						case "system_gift":
							...
							$output .= "received an <a href=\"index.php?title=Special:ViewSystemGift&gift_id={$item["id"]}\" rel=\"nofollow\">award</a> {$item_time}</span>";
							$output .= "<div class=\"user-home-item-gift\">";
							$output .= "<span class=\"user-home-gift-image\">";
							$output .= "<a href=\"index.php?title=Special:ViewSystemGift&gift_id={$item["id"]}\" rel=\"nofollow\">{$gift_image}</a>";
							$output .= "</span>";
							$output .= "<span class=\"user-home-gift-info\">";
							$output .= "{$item["pagetitle"]}";
							$output .= "</span>";
						...
						case "system_message":
							...
							break;
						case "user_message":
							$output .= "wrote on <a href=\"{$user_title_2->getFullURL()}\" rel=\"nofollow\">{$item["comment"]}'s</a> <b><a href=\"" . UserBoard::getUserBoardURL($user_title_2->getText()) . "\" rel=\"nofollow\">board</a></b>  {$item_time}</span>
		...
		$output .= "<div class=\"picture-game-preview\">";
		$output .= $this->getQuizLeaderboardPreview($stats["quiz_points"]);
		$output .= "</div>";
		...
		$output .= "<div class=\"picture-game-preview\">
			<h1>Play the Picture Game</h1>
				<p class=\"main-page-sub-links\">
					...
					<a href=\"index.php?title=Special:PictureGameHome&picGameAction=startGame\" rel=\"nofollow\">Skip This Game</a> -
		...
		if( ($friends_above+$friends_below) > 0){
			...
			foreach($fans_above as $fan) {		
				...
				$topfans .= "<div class=\"top-fan\"> <a href=\"{$user->getFullURL()}\" rel=\"nofollow\">{$avatar->getAvatarURL()}</a> <span class=\"top-fans-user\"><a href=\"{$user->getFullURL()}\" rel=\"nofollow\">{$fan["user_name"]}</a></span> <span class=\"top-fans-points\"><b>+" . number_format($fan["points"])."</b> points</span></div>";
			}
			...
			$topfans .= "<div class=\"top-fan\"> <a href=\"{$user->getFullURL()}\" rel=\"nofollow\">{$avatar->getAvatarURL()}</a> <span class=\"top-fans-user\"><a href=\"{$user->getFullURL()}\" rel=\"nofollow\">{$wgUser->getName()}</a></span> <span class=\"top-fans-points\"><b>+" . number_format($points)."</b> points</span> <span class=\"profile-on\">you</span></div>";
			...
			foreach($fans as $fan) {		
				...
				$topfans .= "<div class=\"top-fan\"><span class=\"top-fan-number\">{$x}.</span> <a href=\"{$user->getFullURL()}\" rel=\"nofollow\">{$avatar->getAvatarURL()}</a> <span class=\"top-fans-user\"><a href=\"{$user->getFullURL()}\" rel=\"nofollow\">{$fan["user_name"]}</a></span> <span class=\"top-fans-points\"><b>+" . number_format($fan["points"])."</b> points</span></div>";
				$x++;
  ...
  function getUserFavoriteTeams($user_id,$user_name){
		...
		if($wgUser->getID()==$user_id) {
			...
			*/
			...
			if ($user_update) {
				...
				$view_thought_link = "<a href=\"" . $thought_link->getFullURL() . "&id={$user_update["id"]}\" rel=\"nofollow\">[see who else agrees]</a>";
				if( $wgUser->isLoggedIn() && $wgUser->getName()!=$user_update["user_name"]){
					if( !$user_update["voted"] ){
						$vote_link = "<a href=\"javascript:void(0);\" onclick=\"vote_status({$user_update["id"]},1)\" rel=\"nofollow\">[agree]</a>";
					}else{
				...
				<p style=\"margin-bottom:20px;\">
					<a href=\"index.php?title=Special:UserStatus&user=" . urlencode($user_name) . "\" rel=\"nofollow\">More Thoughts</a>";
					if($user_name == $wgUser->getName() ){
						$output .= " - <a href=\"index.php?title=Special:UserHome&edits=0&votes=0&comments=0&gifts=0&rel=0&messages=0&system_gifts=0&messages_sent=0&network_updates=1\" rel=\"nofollow\">Friend's Thoughts</a>";
					}
				$output .= "</p>"; 
			} else {
				$output .= "<div id=\"status-update\" class=\"status-message-no\">How sad! {$user_name} has no thoughts.</div>";
				if($user_name == $wgUser->getName() ){
					$output .= "<div style=\"margin-bottom:20px;\"><a href=\"index.php?title=Special:UserHome&edits=0&votes=0&comments=0&gifts=0&rel=0&messages=1&system_gifts=0&messages_sent=0&network_updates=1\" rel=\"nofollow\">Friend's Thoughts</a></div>";
				}
			}
			...
			if($wgUser->getID()==$user_id) {
				$output .= "<a href=\"index.php?title=Special:UpdateFavoriteTeams\" rel=\"nofollow\">Add More Networks</a> - ";
				$output .= "<a href=\"index.php?title=Special:SimilarFans\" rel=\"nofollow\">Meet Similar Fans</a> - ";
			}
			
			$output .= "<a href=\"index.php?title=Special:TopNetworks\" rel=\"nofollow\">Top Networks</a>";
			...
			foreach($favs as $fav){
				...
				if($wgUser->getID()==$user_id) {
					$status_link = " <span class=\"status-message-add\">- <a href=\"javascript:void(0);\" onclick=\"show_message_box({$fav["order"]},{$fav["sport_id"]},{$fav["team_id"]})\" rel=\"nofollow\">add thought</a></span>";
				}
				...
				$output .= "<div class=\"user-fan-networks\">";
					...
					$output .= "<a href=\"{$homepage_title->getFullURL()}&sport_id={$fav["sport_id"]}&team_id={$fav["team_id"]}\" rel=\"nofollow\">{$display_name}</a>";
					$output .= $status_link;
					$output .= "</div>";	
	...
	function getTodaysGames() {
		...
		// for initial display of games (1=MLB)
		$current_featured_sport = 1;
		...
		$todays_games = array();
		$output .= "<div class=\"todays-games\" id=\"todays-games\">
			<h2>Play the Picks Game</h2>";
		
		$script_output .= "<script type=\"text/javascript\">";
		$tabbed_output .= "<div class=\"todays-games-links\">";
		
		//for ($i=1; $i<=sizeof($sports_list); $i++) {
		$sport_count = 0;
		foreach($sports_list as $key=>$value) {
			...
			if ($sport_count==sizeof($sports_list)) {
				$sep="";
			} else {
				$sep=" - ";
			}
			
			if ($i == $current_featured_sport) {
				$featured = "-featured";
				$script_output .= "var featured_sport = \"{$sport_specifics["sport"]}\";";
				//$tabbed_output .= "<a {$featured} href=\"javascript:toggle_upcoming_games_vis('{$sport_specifics["sport"]}');\">{$sport_specifics["sport"]}</a>{$sep}";
				$tabbed_output .= "<span id=\"todays-games-tab-{$sport_specifics["sport"]}\" style=\"font-weight: bold;\">{$sport_specifics["sport"]}</span>{$sep}";
			}
			else {
				$featured = "";
				$tabbed_output .= "<span id=\"todays-games-tab-{$sport_specifics["sport"]}\"><a href=\"javascript:toggle_upcoming_games_vis('{$sport_specifics["sport"]}');\" rel=\"nofollow\">{$sport_specifics["sport"]}</a></span>{$sep}";
			}
			//$tabbed_output .= "<a {$featured} href=\"javascript:toggle_upcoming_games_vis('{$sport_specifics["sport"]}');\">{$sport_specifics["sport"]}</a>{$sep}";
			$games_output .= "<div class=\"todays-games-sport{$featured}\" id=\"todays-games-{$sport_specifics["sport"]}\">";
			
			$key = wfMemcKey( 'todays', 'games', $sport_specifics["sport"] );
			$data = $wgMemc->get( $key );
			
			//no cache, load from db
			if(!$data){
				wfDebug( "loading front page {$sport_specifics["sport"]} pick game data from db\n" );
				...
				wfDebug( "loading front page {$sport_specifics["sport"]} pick game data from cache\n" );
				$list = $data;
			}
			...
			for ($j=0; $j<sizeof($list); $j++) {
				...
				$row = $list[$j];
				 ...
				 $games_output .= "<div class=\"todays-games-game\">";
				 $games_output .= "<div class=\"todays-games-date\">" . date("D, M j - g:i A", $row->pick_game_timestamp) . " ({$row->votes} picks)</div>";
				 $games_output .= "<div class=\"todays-games-matchup\">
				 <div class=\"todays-games-visitor\" onclick=\"window.location='{$link_location}'\"  onmouseover=\"imageSwap('todays-games-up-arrow-{$row->pick_game_id}','up-arrow',1)\" onmouseout=\"imageSwap('todays-games-up-arrow-{$row->pick_game_id}','up-arrow',0)\"><img id=\"todays-games-up-arrow-{$row->pick_game_id}\" src=\"images/common/up-arrow.gif\">{$row->pick_game_visitor}{$sport_specifics["additional_delimiter"]}{$row->pick_visitor_addl}</div>
				 <div class=\"todays-games-home\" onclick=\"window.location='{$link_location}'\" onmouseover=\"imageSwap('todays-games-down-arrow-{$row->pick_game_id}','down-arrow',1)\" onmouseout=\"imageSwap('todays-games-down-arrow-{$row->pick_game_id}','down-arrow',0)\"><img id=\"todays-games-down-arrow-{$row->pick_game_id}\" src=\"images/common/down-arrow.gif\">{$row->pick_game_home}{$sport_specifics["additional_delimiter"]}{$row->pick_home_addl}</div>
				 </div></div>";
				 $count++;
			}
			if (sizeof($list) === 0) {
				$games_output .= "<div class=\"todays-games-nogame\">There are no upcoming {$sport_specifics["sport"]} Games.  Please try a different sport</div>";
			}
			/*
			$res = $dbr->query($sql);
			$count = 1;
			...
			while ($row = $dbr->fetchObject( $res ) ) {
				 ...
				 $games_output .= "<div class=\"todays-games-date\">" . date("D, M j - g:i A", $row->pick_game_timestamp) . " ({$row->votes} picks)</div>";
				 $games_output .= "<div class=\"todays-games-matchup\">
				 <div class=\"todays-games-visitor\" onclick=\"window.location='index.php?title=Special:PickGame/{$i}'\"  onmouseover=\"imageSwap('todays-games-up-arrow-{$row->pick_game_id}','up-arrow',1)\" onmouseout=\"imageSwap('todays-games-up-arrow-{$row->pick_game_id}','up-arrow',0)\"><img id=\"todays-games-up-arrow-{$row->pick_game_id}\" src=\"images/common/up-arrow.gif\">{$row->pick_game_visitor}{$sport_specifics["additional_delimiter"]}{$row->pick_visitor_addl}</div>
				 <div class=\"todays-games-home\" onclick=\"window.location='index.php?title=Special:PickGame/{$i}'\" onmouseover=\"imageSwap('todays-games-down-arrow-{$row->pick_game_id}','down-arrow',1)\" onmouseout=\"imageSwap('todays-games-down-arrow-{$row->pick_game_id}','down-arrow',0)\"><img id=\"todays-games-down-arrow-{$row->pick_game_id}\" src=\"images/common/down-arrow.gif\">{$row->pick_game_home}{$sport_specifics["additional_delimiter"]}{$row->pick_home_addl}</div>
				 </div></div>";
				 $count++;
		...
		$script_output .= "function toggle_upcoming_games_vis(sport) {";
		$script_output .= "var tab_base = 'todays-games-tab-';";
		$script_output .= "var games_base = 'todays-games-';";
		$script_output .= "\$(tab_base + featured_sport).setStyle({fontWeight: 'normal'});";
		$script_output .= "\$(tab_base + featured_sport).innerHTML = \"<a href=javascript:toggle_upcoming_games_vis('\" + featured_sport + \"');>\" + featured_sport + \"</a>\";";
		$script_output .= "\$(games_base + featured_sport).setStyle({display: 'none'});";
		$script_output .= "\$(games_base + featured_sport).setStyle({visibility: 'hidden'});";
		$script_output .= "\$(tab_base + sport).innerHTML = sport;";
		$script_output .= "\$(tab_base + sport).setStyle({fontWeight: 'bold'});";
		$script_output .= "\$(games_base + sport).setStyle({display: 'inline'});";
		$script_output .= "\$(games_base + sport).setStyle({visibility: 'visible'});";		
		$script_output .= "featured_sport = sport;}";
		$script_output .= "function imageSwap(divID, type, on) {";
		$script_output .= "if (on==1) {";
		$script_output .= "\$(divID).src='images/common/'+type+'-on.gif';";
		$script_output .= "} else {";
		$script_output .= "\$(divID).src='images/common/'+type+'.gif';";
		$script_output .= "}";
		$script_output .= "}";
		$script_output .= "</script>";
			
		
		$output .= $tabbed_output . $games_output . $script_output;
		$output .= "</div>";
