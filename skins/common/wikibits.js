...
function mwWikiaUploadButton(namespace, tooltip) {
	...
	upload_image.src = "/skins/common/images/button_upload.gif";
...
function mwSetupToolbar() {
	...
	mwWikiaUploadButton(mwWikiaUploadNamespace, mwWikiaUploadTooltip);
...
function loginBeforeUpload(tagOpen, tagClose, sampleText)
{
   var domainRE = /http:\/\/([^\/]*)\/index\.php\?title\=(.*)\&action\=edit/g;
   var matches = domainRE.exec(window.location.href);
   var domain = matches[1];
   var articleName = matches[2];

   var result = confirm("You need to be logged in to upload an image file.  Do you want to login now?");
   if ( result == true ) {

      var loginPageURL = "http://" + domain + "/";
      if ( articleName && articleName.length > 0 )
        loginPageURL += "index.php?title=Special:Userlogin&returnto="+articleName;
      else
        loginPageURL += "wiki/Special:Userlogin";
