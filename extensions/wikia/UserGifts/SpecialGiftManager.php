...
function wfSpecialGiftManager(){
	...
	class GiftManager extends SpecialPage {
		...
		function canUserCreateGift(){
			...
			if( in_array('giftadmin',($wgUser->getGroups())) || ($wgMaxCustomUserGiftCount > 0 && $created_count < $wgMaxCustomUserGiftCount ) ){
				return true;
			}else{
				return false;
			}
			...
			if($gifts){
				foreach ($gifts as $gift) {
					$output .= "<div class=\"Item\" >
							<a href=\"index.php?title=Special:GiftManager&amp;id={$gift["id"]}\">{$gift["gift_name"]}</a>
						</div>\n";
				}
			}
			$output .= "</div>";
		...
		function displayForm($gift_id){
			...
			if($gift_id){
				$gift_image = "<img src=\"/images/awards/" . Gifts::getGiftImage($gift_id,"l") . "\" border=\"0\" alt=\"gift\" />";
				$form .=  '<tr>
