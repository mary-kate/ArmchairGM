	...
	function execute($value) {
		...
		if (!$value) {
			...
			//if (isset($group_info["creator"]) && $group_info["creator"] == $current_user_id) {
				...
				$group_info = MarchMadness::get_group_info_from_db($entry_info["group_id"]);
				
				if($group_info["creator"] ==$current_user_id) {
					
					$output .= "<div class=\"madness-group-update\">";
					
					$output .= "<input type=\"text\" id=\"group_name\" value=\"{$group_info["group_name"]}\" maxlength=\"50\" /><br/>";
					$output .= "<textarea id=\"group_desc\">{$group_info["group_description"]}</textarea><br/>";
					$output .= "<input type=\"button\" class=\"site-button\" value=\"update group info\" onclick=\"update_group_name({$entry_info["group_id"]}, $value);\" /><br/>";
