	...
	function execute(){
		...
		*/
		...
		$sql = "SELECT count( * ) AS the_count, Date_FORMAT( `ur_date` , '%y %m' ) as the_date
			...
			LIMIT 0,6";
		$res = $dbr->query($sql);
		$output .= $this->display_stats("NEW USERS (since mid-late May)",$res);
		...
		$sql = "SELECT count( * ) AS the_count, Date_FORMAT( `ur_date` , '%y %m' ) as the_date
			...
			LIMIT 0,6";
		$res = $dbr->query($sql);
		$output .= $this->display_stats("RECRUITS (since mid-late May)",$res);
		...
		$sql = "SELECT sum( ue_count ) AS the_count, Date_FORMAT( `ue_date` , '%y %m' ) as the_date
			...
			LIMIT 0,6";
		$res = $dbr->query($sql);
		$output .= $this->display_stats("INVITES VIA CONTACT IMPORTER (July 3rd)",$res);
		...
		$sql = "SELECT sum( ue_count ) AS the_count, Date_FORMAT( `ue_date` , '%y %m' ) as the_date
			...
			LIMIT 0,6";
		$res = $dbr->query($sql);
		$output .= $this->display_stats("INVITES TO READ NEW OPINION (July 3rd)",$res);		
