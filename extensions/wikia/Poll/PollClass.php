...
class Poll {
	...
	public function user_voted($user_name,$poll_id){
		$dbr =& wfGetDB( DB_SLAVE );
		$s = $dbr->selectRow( '`poll_user_vote`', array( 'pv_id' ), array('pv_poll_id'=>$poll_id, 'pv_user_name' => $user_name ), $fname );
		if ( $s !== false ) {
			return true;
		}
		return false;
	...
	public function does_user_own_poll($user_id,$poll_id){
		$dbr =& wfGetDB( DB_SLAVE );
		$s = $dbr->selectRow( '`poll_question`', array( 'poll_id' ), array('poll_id'=>$poll_id, 'poll_user_id' => $user_id ), $fname );
		if ( $s !== false ) {
			return true;
		}
		return false;
