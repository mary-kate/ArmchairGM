...
function wfSpecialUpdatePoll(){
	...
	class UpdatePoll extends SpecialPage {
		...
		function execute(){
			...
			global $wgUser, $wgOut, $wgRequest, $IP, $wgMemc, $wgStyleVersion;
		...
		function displayForm(){
			...
			$form .= "<div class=\"update-poll-left\">
				...
				foreach($poll_info["choices"] as $choice){
					$form .= "<div class=\"update-poll-answer\"><span class=\"update-poll-answer-number\">{$x}.</span> <input type=\"text\" tabindex=\"{$x}\" id=\"poll_answer_{$x}\" name=\"poll_answer_{$x}\" value=\"{$choice["choice"]}\"/></div>";
					$x++;
				}
