<?php
$wgExtensionFunctions[] = "wfExtensionSpecialRCSimple";

function wfExtensionSpecialRCSimple()
{
    //global $wgMessageCache;
    //$wgMessageCache->addMessages(array('hello' => 'Hello page'));

     global $wgUser,$IP,$wgMessageCache;
     include_once("includes/SpecialPage.php");
     include_once("includes/ChangesList.php");
     $wgMessageCache->addMessages( array('rc_simple_title' => 'Recent Changes (simple)'), 'en' );
     
     
     class RecentChangesSimple extends SpecialPage {

	var $included_files;
	
	function RecentChangesSimple(){
		UnlistedSpecialPage::UnlistedSpecialPage("RecentChangesSimple");
	}
	...
	function rcToggle() {
		...
		if( ! $wgUser->isLoggedIn() && isset($_COOKIE[$wgCookiePrefix . '_usenewrc']) ) {
		    $wgUser->setOption( 'usenewrc', ($_COOKIE[$wgCookiePrefix . '_usenewrc'] == 'yes' ? 1 : 0));
		}
	
		$toggle = $wgRequest->getVal( 'toggle' );
		if( $toggle ) {
		    $wgUser->setOption( 'usenewrc', ! $wgUser->getOption( 'usenewrc' ) );
		    $wgUser->saveSettings();
		    if( ! $wgUser->isLoggedIn() ) {
			    setcookie( $wgCookiePrefix.'_usenewrc', ($wgUser->getOption( 'usenewrc' ) ? 'yes' : 'no'));
		    }
	
		    SquidUpdate::purge( array( $_SERVER['HTTP_REFERER'] ) );
	...
	function execute($par){
		...
		if( $feedFormat ) {
			...
			if ( !$this->including() ) {
				...
				$toggleLink = '';
				$wgOut->addHTML( '<div class="rcoptions">' . $this->rcOptionsPanel( $defaults, $nondefaults, $toggleLink ) . "\n" );
				$wgOut->addHTML( $this->rcNamespaceForm( $namespace, $invert, $nondefaults, $any ) . $toggleLink . '</div>'."\n");
			}
			...
			//******CHANGED HERE jt *********//
			$list = new SimpleChangesList( $wgUser->getSkin() );
			...
			$wgOut->setPageTitle( wfMsg('rc_simple_title') );
			$wgOut->addHTML( $s );
		}
		 ...
		 * @param $nondefaults
		 */
		function rcOptionsPanel( $defaults, $nondefaults, &$toggleLink ) {
			...
			// show toggle link
			global $wgMessageCache, $wgUser, $wgContLang;
			$wgMessageCache->addMessage ('toggle_enhanced', 'Toggle Enhanced Recent Changes');
			$sk = $wgUser->getSkin();
			$toggleLink = $sk->makeKnownLink( $wgContLang->specialPage( 'Recentchanges' ), wfMsg('toggle_enhanced'), wfArrayToCGI( array('toggle' => 1), $nondefaults ), '', '', 'style="font-size: 80%"' );
		...
		function rcApplyDiffStyle( $text ) {
     	...
     	/**
	 * Generate a list of changes using the Simple
	 */
	class SimpleChangesList extends ChangesList {
		/**
		...
		function insertDiffHist(&$s, &$rc, $unpatrolled) {
			...
			if( $rc->mAttribs['rc_type'] == RC_NEW || $rc->mAttribs['rc_type'] == RC_LOG ) {
				...
				$rcidparam = $unpatrolled
					...
					: array();
				$diffLink = $this->skin->makeKnownLinkObj( $rc->getTitle(), 'changes',
					wfArrayToCGI( array(
		...
		function insertUserRelatedLinks(&$s, &$rc) {
			...
			$s .= ' (' . $wgOut->parse( '[['.User::newFromId($rc->mAttribs['rc_user'])->getTalkPage()->getFulltext() . '|talk]]', false ) .')';
		}
		...
		function insertDateHeader(&$s, $rc_timestamp) {
			...
			if( $date != $this->lastdate ) {
				...
				$s .= "<tr><td colspan=6><h4>{$date}</h4></td></tr>";
				$this->lastdate = $date;
		...
		function beginRecentChangesList() {
			...
			return '<table width=100%><tr><td width=10%>&nbsp;</td><td width=3%>&nbsp;</td><td width=29%><b>Page</b></td><td width=5%><b>Time</h4></b><td width=15%><b>Author</b></td><td width=35%><b>Comment</b></td></tr>';
		}
		...
		function showPreview(&$s, $rc) {
			global $wgOut, $IP, $wgRcSimplePreviewList;
			
			if ($wgRcSimplePreviewList && is_array($wgRcSimplePreviewList)) {
				$prev_array = $wgRcSimplePreviewList;
			}
			else {
				$prev_array = array();
			}
			...
			$prev_array = array(
				NS_FANTAG=>array(
					'file'=>$IP.'/extensions/wikia/Rob/FanBoxClass.php',
					'constructor'=>'FanBox::newFromName',
					'constructor_type'=>'title',
					'preview_funcs'=>array('outputFanBox'=>'')
					),
				NS_IMAGE=>1,
				NS_USER=>1
				);
			*/
			$current_ns = $rc->getTitle()->getNamespace();
			if ($current_ns && (isset($prev_array[$current_ns]) || in_array($current_ns, $prev_array))) {
				
				if (is_array($prev_array[$current_ns]) && isset($prev_array[$current_ns]["file"]) && !isset($this->included_files[$current_ns])) {
					$this->included_files[$current_ns] = $prev_array[$current_ns]["file"];
					include_once($prev_array[$current_ns]["file"]);
					//$s .= "including " . $prev_array[$current_ns]["file"];
				}
				if (is_array($prev_array[$current_ns]) && isset($prev_array[$current_ns]["preview_funcs"])) {
					
					if(isset($prev_array[$current_ns]["constructor"])) {
						//$constructor_param = "'" . $rc->getTitle()->getText() . "'";
						/* need to add later*/
						if (isset($prev_array[$current_ns]["constructor_type"])) {
							switch ($prev_array[$current_ns]["constructor_type"]) {
								case 'id':
									$constructor_param = $rc->getTitle()->getArticleID();
									break;
								default:
									$constructor_param = "'" . $rc->getTitle()->getText() . "'";
							}
						...
						//$s .= $constructor_param;
						$func = $prev_array[$current_ns]["constructor"] . "(" . $constructor_param . ")";
						$obj = eval('return ' . $func . ';');
						
						if (isset($prev_array[$current_ns]["prep_funcs"]) && is_array($prev_array[$current_ns]["prep_funcs"])) {
							foreach ($prev_array[$current_ns]["prep_funcs"] as $keyprep=>$valueprep) {
								if ($value && strlen($valueprep) && strpos($value, "(") ===0) {
									$pointer = "->";
								}
								else {
									$pointer = "";
								}
								//$func = "\$obj{$pointer}" . $keyprep . $valueprep;
								$func = $keyprep . $valueprep;
								$obj = eval('return ' . $func . ';');
							}
						}
						
						foreach ($prev_array[$current_ns]["preview_funcs"] as $key=>$value) {
							if ($value && strlen($value) && strpos($value, "(") ===0) {
								$pointer = "->";
							}
							else {
								$pointer = "";
							}
							$func = "\$obj{$pointer}" . $key . $value;
							//$func = $key . $value;
							
							$output = eval('return ' . $func . ';');
							if (isset($prev_array[$current_ns]["text_limit"]) && is_numeric($prev_array[$current_ns]["text_limit"])) {
								$output = substr($output, 0, $prev_array[$current_ns]["text_limit"]) . ((strlen($output) > $prev_array[$current_ns]["text_limit"]) ? "..." : "");
					...
					switch ($current_ns) { 
						/*case NS_USER:
							$id=User::idFromName($rc->getTitle()->getText());
							$avatar = new wAvatar($id,"m");
							//$avatarID = $avatar->getAvatarImage();
							//$s .= " <a href=\"" . $rc->getTitle()->getFullURL() . "\"><img src=/images/avatars/{$avatarID} style=\"border:1px solid #d7dee8;\"/></a>";
							$s .= $avatar->getAvatarURL();
							break;
							*/
						default:
							$s .= $wgOut->parse( '[[' . $rc->getTitle()->getFullText() . ']]', false);
				...
				if ( $specialName == 'Log' ) {
					...
					$re_pattern = "/\[\[([^\]]*)\]\]/";
					preg_match($re_pattern, $rc->mAttribs['rc_comment'], $matches);
					if (sizeof($matches)) {
						if (Title::newFromText($matches[1])->getNameSpace() && (isset($prev_array[Title::newFromText($matches[1])->getNameSpace()]) || in_array(Title::newFromText($matches[1])->getNameSpace(), $prev_array))) {
							if (Title::newFromText($matches[1])->getNameSpace() == NS_IMAGE) {
								$image_display = Image::newFromTitle(Title::newFromText($matches[1]));
								$image_thumb = $image_display->getThumbnail( 50, -1, true );
								$s .= $image_thumb->toHtml();
								//$s .= $wgOut->parse( '[[' . $matches[1] . ']]', false);
							}
							else {
								$s .= $wgOut->parse( '[[' . $matches[1] . ']]', false);
							}
					...
					/*if (strpos($rc->mAttribs['rc_comment'], "uploaded \"") === 0) {
						$s .= $wgOut->parse(substr($rc->mAttribs['rc_comment'], 10, strlen($rc->mAttribs['rc_comment'])-11), false);
					}*/
					//$this->insertLog( $s, $rc->getTitle(), $specialSubpage );
				} else {
					//wfDebug( "Unexpected special page in recentchanges\n" );
				...
				//$s .= $rc->getTitle()->getNamespace();
			}
		}
	}
     
    SpecialPage::addPage( new RecentChangesSimple );
}
