...
require_once( '/srv/web/armchair/docroots/sports/includes/ChangesList.php' );
...
function wfSpecialRecentchanges( $par, $specialPage ) {
	...
	if( $feedFormat ) {
		...
		if ( !$specialPage->including() ) {
			...
			$toggleLink = '';
			$wgOut->addHTML( '<div class="rcoptions">' . rcOptionsPanel( $defaults, $nondefaults, $toggleLink ) . "\n" );
			$wgOut->addHTML( rcNamespaceForm( $namespace, $invert, $nondefaults, $any ) . $toggleLink . '</div>'."\n");
		}
