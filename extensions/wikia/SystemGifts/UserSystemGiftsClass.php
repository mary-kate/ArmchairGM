...
class UserSystemGifts {
	...
	public function sendGiftNotificationEmail($user_id_to,$gift_id ){    
		...
		if($user->isEmailConfirmed() && $user->getIntOption("notifygift",1) ){
			...
			$subject = wfMsg( 'system_gift_received_subject',
				$gift["gift_name"]
				 );
			$body = wfMsgForContent( 'system_gift_received_body', 
	...
	static function getNewSystemGiftCountDB($user_id){
		...
		$dbr =& wfGetDB( DB_SLAVE );
		$new_gift_count = 0;
		$s = $dbr->selectRow( '`user_system_gift`', array( 'count(*) as count' ), array( 'sg_user_id' => $user_id, 'sg_status' => 1 ), $fname );
