...
class Comment{
	...
	function getCommentText($comment_text){
		...
		$comment_text = $CommentParser->parse($comment_text_fix, $wgTitle, $wgOut->parserOptions(), true );
							...
							if ($this->AllowMinus == true || $this->AllowPlus == true) {
								...
								if (!$wgReadOnly) {
									...
									if (!in_array( $comment["CommentID"], $voted)) {
										...
										$output .= "<img src=\"".$wgUploadPath."/myfeed.gif\" border=\"0\" alt=\"\"/>".wfMsg("comment_voted_label");
									}
								}
								$output .= "</span>";
							}
