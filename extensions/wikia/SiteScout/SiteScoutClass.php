	...
	function getControls(){	
		...
		$output .= "<script >
			function changeFilter(){
				if(!\$(\"f_edits\").checked){edits=0}else{edits=1};
				if(!\$(\"f_votes\").checked){votes=0}else{votes=1};
				if(!\$(\"f_comments\").checked){comments=0}else{comments=1};
				if(\$(\"f_networkupdates\") && !\$(\"f_networkupdates\").checked){networkupdates=0}else{networkupdates=1};
				document.cookie='scout_edits='+edits
				document.cookie='scout_votes='+votes
				document.cookie='scout_comments='+comments
				document.cookie='scout_networkupdates='+networkupdates
				window.location=\"index.php?title=Special:SiteScout&edits=\" + edits+\"&votes=\" + votes+\"&comments=\" + comments+\"&networkupdates=\" + networkupdates
			}
			</script>";
