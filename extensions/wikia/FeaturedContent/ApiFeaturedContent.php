...
// See http://staff.wikia.com/wiki/User:Ppiotr/Gamespot_project#Featured_content for feat.cont. cookbook

class ApiFeaturedContent extends ApiBase
{
	const MEMC_KEY  = 'api:featuredcontent';
	const MEMC_TIME =  5400; // 90 min FIXME add cache-clearing FeaturedContent_setup.php and bump it up to 30 days
	...
	public function execute()
	{
		...
		$key = wfMemcKey(self::MEMC_KEY);
		wfDebug(sprintf("%s: key: %s\n", __METHOD__, $key));

		global $wgMemc;
		$data = $wgMemc->get($key);
		if (empty($data))
		{  
			wfDebug(sprintf("%s: cache was empty\n", __METHOD__));
			$data = $this->getData();
			$wgMemc->set($key, $data, self::MEMC_TIME);
		} else
		{  
			wfDebug(sprintf("%s: cache is full\n", __METHOD__));
		}
	...
	protected function getData()
	{
		...
		//wfDebug(sprintf("FeaturedContent: API: getData: %s\n", $list));
		...
		if (preg_match_all('/(?:<li>(.+)<\/li>|<tr>\s*<td>(.+)<\/td>\s*<\/tr>)/sU', $list, $preg, PREG_SET_ORDER))
		{
			...
			{
				// its preg( $match[1] || $match[2] ) so we may as well add them instead of going through a bunch of ifs
				$content2 = trim($match[1].$match[2]);
					 ...
					 preg_match('/^(.*)<a href="([^"]+)"[^>]*title="([^"]+)"[^>]*>.+<\/a>(.*)$/sU', $content2, $preg2))
				{
					...
					if (!empty($article) || !empty($snippet))
					{
						$row = array
						(
							'title'     => $article,
		...
		*/
		...
		foreach ($data as $row)
		{
			...
			/* its a hack, a better way would be to make this API call a generator */
			$last_edit = $this->getLastEditTimestamp($row['title']);
	...
	static public function getMinimalHtml($text, $N_words = 100)
	{
		...
		wfDebug(sprintf("%s: text: %s\n", __METHOD__, $text));

		$text = preg_replace('/<span class="editsection">\s*\[[^\]]+\]\s*<\/span>/sU', '', $text);
		wfDebug(sprintf("%s: after preg edit: %s\n", __METHOD__, $text));
		$text = preg_replace('/<h[0-9]>.*<span class="mw-headline">\s*([^<]+)\s*<\/span>.*<\/h[0-9]>/sU', '', $text);
		wfDebug(sprintf("%s: after preg headline: %s\n", __METHOD__, $text));

		// this is a mess... )-: don't overdo regexp, just add another one, if needed
		$text = preg_replace('/<div class="thumb tright"><div class="thumbinner"[^>]*><a [^>]*><img [^>]*><\/a>\s*<div class="thumbcaption"><div class="magnify"[^>]*><a [^>]*><img [^>]*><\/a><\/div>.*<\/div><\/div><\/div>/sU', '', $text);
		wfDebug(sprintf("%s: after preg right thumb + caption: %s\n", __METHOD__, $text));

		$text = strip_tags($text, '<p><br><i><b>');
		wfDebug(sprintf("%s: after strip: %s\n", __METHOD__, $text));
		$text = trim($text);
		wfDebug(sprintf("%s: after trim: %s\n", __METHOD__, $text));
	...
	protected function _parseTimestamps($data)
	{
		...
		foreach ($data as $row)
		{
			...
			if (preg_match("/({$months_mask}) ([0-9]{1,2}), ([0-9]{4})/", $row, $preg))
			{
				...
				$day   = $preg[2];
			} elseif (preg_match('/([0-9]{4})-([0-9]{1,2})-([0-9]{1,2})/', $row, $preg))
			{
				$year  = $preg[1];
				$month = $preg[2];
				$day   = $preg[3];
	...
	static public function getHeadlineFromHtml($content)
	{
		...
		$headline = '';
		if (preg_match('/<h[0-9]>.*<span class="mw-headline">\s*([^<]+)\s*<\/span>.*<\/h[0-9]>/sU', $content, $preg))
		{
	...
	static public function getLastEditTimestamp($title)
	{
		wfProfileIn(__METHOD__);
		...
		{
			$article   = new Article($title);
			$last_edit = $article->getTimestamp();
			$last_edit = wfTimestamp(TS_ISO_8601, $last_edit);
		}
