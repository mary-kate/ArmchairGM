...
/* the core */
function wfRegexBlockSpecial () {
	global $wgOut, $wgUser, $wgRequest ;
   	$wgOut->setPageTitle("Regular Expression Name Block");
	$rBS = new regexBlockForm () ;
	$rBL = new regexBlockList () ;
...
function wfRegexBlockUnsetKeys ($blocker, $username) {
	...
	$wgMemc->delete ("$wgSharedDB:regexBlockCore:blocker:$blocker") ;
...
class regexBlockList {
	...
	function showSuccess () {
		...
		$wgOut->addWikiText('User name or IP address <b>'.htmlspecialchars($wgRequest->getVal('ip')).'</b> has been unblocked.') ;
	}
...
class regexBlockForm {
	...
	/* constructor */
	function regexBlockForm () {
		global $wgRequest ;
		$this->mRegexBlockedAddress = $wgRequest->getVal( 'wpRegexBlockedAddress',  $wgRequest->getVal( 'ip') );
...
<form name=\"regexblock\" method=\"post\" action=\"{$action}\">
	<table border=\"0\">
		<tr>
			...
			<td align=\"left\">
				<input tabindex=\"1\" name=\"wpRegexBlockedAddress\" id=\"wpRegexBlockedAddress\" size=\"40\" value=\"{$scRegexBlockedAddress}\" />
			</td>
		</tr>
		<tr>
			<td align=\"right\">Reason:</td>
			<td align=\"left\">
				<input tabindex=\"2\" name=\"wpRegexBlockedReason\" id=\"wpRegexBlockedReason\" size=\"40\" value=\"{$scRegexBlockedReason}\" />
			</td>
		</tr>
                <tr>
	...
	function doSubmit () {
		...
		$query = "REPLACE INTO ".wfRegexBlockGetTable()." 
