...
class FanBoxes extends SpecialPage {
	...
	function FanBoxes(){
		UnlistedSpecialPage::UnlistedSpecialPage("FanBoxes");		
	}
...
//colorpicker
	...
	if( !$wgUser->isLoggedIn() ){
				...
				//$output = "You must log-in to create a fan box.";
                                $output .= wfMsgForContent('fan_createfannotloggedin');
				$output .= "<p>
					<input type=\"button\" class=\"site-button\" onclick=\"window.location='index.php?title=Special:UserRegister'\" value=\"Sign Up\"/>
					<input type=\"button\" class=\"site-button\" onclick=\"window.location='index.php?title=Special:Login&returnto=Special:FanBoxes'\" value=\"Log In\"/>
				</p>";
		...
		if($fanboxid){
			...
			while ($row = $dbr->fetchObject( $res ) ) {
				if( $row->cl_to != "Fans" && strpos($row->cl_to, "Fans_by_User_" ) === false  ){
		...
		if($fanboxid)$page_title=wfMsgForContent( 'fan_updatefan' )." for ".str_replace("_", " ", $update_fan-> getName());
		...
		$output .="<div class=\"create-fanbox-text-and-color\">
		...
		$output .="<div class=\"fanbox-item\">
			<div class=\"individual-fanbox\" id=\"individualFanbox\">
			...
			$output.= "<h1>". wfMsgForContent( 'fanbox_addtext' ) ."</h1>
				<div class=\"create-fanbox-text\">
					<div id=\"fanbox-left-text\">
						...
						<input type=\"text\" name=\"inputLeftSide\" id=\"inputLeftSide\" oninput=\"displayLeftSide(); leftSideFanBoxFormat()\" onkeyup=\"displayLeftSide(); leftSideFanBoxFormat()\" onkeydown=\"displayLeftSide(); leftSideFanBoxFormat()\"  
						maxlength=\"11\"><br>
						<font size=\"1\">". wfMsgForContent ('fanbox_leftsideinstructions')."</font>
					</div>
					<div id=\"fanbox-right-text\">
					<h3>". wfMsgForContent( 'fanbox_rightsidetext' ) ."<span id=\"countdownbox\"> <font size=\"1\">". wfMsgForContent ('fanbox_youhave')."<input readonly type=\"text\" name=\"countdown\" style=\"width:20px; height:15px;\" value=\"70\"> ". wfMsgForContent ('fanbox_charactersleft')."</font></span></h3> 
		...
		if($fanboxid){
				...
				<div class=\"create-fanbox-text\">
					<div id=\"fanbox-left-text\">
						<h3>". wfMsgForContent( 'fanbox_leftsidetext' ) ."<span id=\"addImage\" onclick=\"displayAddImage('create-fanbox-image', 'addImage', 'closeImage')\">". wfMsgForContent( 'fanbox_display_image' ) ."</span> <span id=\"closeImage\" onclick=\"displayAddImage('create-fanbox-image', 'closeImage', 'addImage')\">". wfMsgForContent( 'fanbox_close_image' ) ."</span></h3>
						<input type=\"text\" name=\"inputLeftSide\" id=\"inputLeftSide\" value=\"{$update_fan->getFanBoxLeftText()}\" oninput=\"displayLeftSide(); leftSideFanBoxFormat()\" onkeyup=\"displayLeftSide(); leftSideFanBoxFormat()\" onkeydown=\"displayLeftSide(); leftSideFanBoxFormat()\"  
						maxlength=\"11\"><br>
						<font size=\"1\">". wfMsgForContent ('fanbox_leftsideinstructions')."</font>
					</div>
					<div id=\"fanbox-right-text\">
						<h3>". wfMsgForContent( 'fanbox_rightsidetext' ) ."<font size=\"1\">". wfMsgForContent ('fanbox_youhave')."<input readonly type=\"text\" name=\"countdown\" style=\"width:20px; height:15px;\" value=\"70\"> ". wfMsgForContent ('fanbox_charactersleft')."</font></h3> 
		...
		if($fanboxid){
			...
			$output.="<script>
						...
						function reset_upload(){
							\$('imageUpload-frame').src=\"?title=Special:FanBoxAjaxUpload\"
