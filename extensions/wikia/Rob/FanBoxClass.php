...
class FanBox{
	...
	public function getBaseCategories(){
		...
		$ctg = "";
		$ctg .= "[[" . $wgContLang->getNsText( NS_CATEGORY ) . ":Fans]]\n";
		$ctg .= "[[" . $wgContLang->getNsText( NS_CATEGORY ) . ":Fans by User {$creator}]]\n";
		return $ctg;
	}
	...
	public function outputFanBox(){
		...
		$output.="<input type=\"hidden\" name=\"individualFantagId\" value=\"$this->getFanBoxId()\">
			<div class=\"individual-fanbox\" id=\"individualFanbox\">
				...
				$fantag_perma
				<table  class=\"fanBoxTable\" onclick=\"javascript:openFanBoxPopup('fanboxPopUpBox','individualFanbox')\" border=\"0\" cellpadding=\"0\" cellspacing=\"0\" >
				<tr><td id=\"fanBoxLeftSideOutput\" style=\"color:".$this->getFanBoxLeftTextColor()." ;font-size:$leftfontsize\" bgcolor=\"".$this->getFanBoxLeftBgColor()."\">".$fantag_leftside."</td> 
				<td id=\"fanBoxRightSideOutput\" style=\"color:".$this->getFanBoxRightTextColor()." ;font-size:$rightfontsize\" bgcolor=\"".$this->getFanBoxRightBgColor()."\">".$this->getFanBoxRightText()."</td>
				</table>
				</div>
			</div>		
		";
	...
	public function outputIfUserHasFanBox() {
		...
		$output .= "
			<div class=\"fanbox-pop-up-box\" id=\"fanboxPopUpBox\">
			<table cellpadding=\"0\" cellspacing=\"0\" width=\"258px\"><tr><td align=\"center\">". wfMsgForContent( 'fanbox_remove_fanbox' ) ."<tr><td align=\"center\">
			<input type=\"button\" value=\"Remove\" size=\"20\" onclick=\"closeFanboxAdd('fanboxPopUpBox','individualFanbox'); showMessage(2, '$fanboxtitle', $individual_fantag_id) \" />
			<input type=\"button\" value=\"Cancel\" size=\"20\" onclick=\"closeFanboxAdd('fanboxPopUpBox','individualFanbox')\" />
			</td></table>
			</div>";
	...
	public function outputIfUserDoesntHaveFanBox() {
		...
		$output .= "
			...
			<input type=\"button\" value=\"Add\" size=\"20\" onclick=\"closeFanboxAdd('fanboxPopUpBox','individualFanbox'); showMessage(1, '$fanboxtitle', $individual_fantag_id) \"/>
			<input type=\"button\" value=\"Cancel\" size=\"20\" onclick=\"closeFanboxAdd('fanboxPopUpBox','individualFanbox')\" />
			</td></table>
			</div>";
	...
	public function getEmbedThisCode() {
		return "[[Fan:{$this->getName()}]]";
		//return "<fan name=\"{$this->getName()}\"></fan>";
	}
