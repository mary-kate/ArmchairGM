...
$wgExtensionFunctions[] = 'wfSpecialLinkApprove';

function wfSpecialLinkApprove(){
	global $wgUser,$IP;
	...
	class LinkApprove extends UnlistedSpecialPage {
		...
		function canAccess(){
			...
			if( in_array( "staff", $wgUser->getGroups() ) || in_array( "linkadmin", $wgUser->getGroups() ) ){
				return true;
			}
		...
		function execute(){
			...
			$wgOut->addScript("<script type=\"text/javascript\" src=\"/extensions/wikia/LinkFilter/LinkFilter.js?{$wgStyleVersion}\"></script>\n");
			...
			foreach($links as $link){
				...
				$output .= "<div class=\"admin-link {$border_fix}\">
						...
						<div class=\"admin-submitted\"><b>" . wfMsg("linkfilter-submittedby") . "</b>: <a href=\"{$submitter->escapeFullURL()}\">{$link["user_name"]}</a> " . get_time_ago($link["timestamp"]) . " " . wfMsg("time_ago") . " under <i>" . Link::$link_types[$link["type"]] . "</i></div>
	...
	SpecialPage::addPage( new LinkApprove );
}
