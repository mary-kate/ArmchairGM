...
You will also need the following:
	* PHP 5. Yes, really. I use a couple things that won't work on PHP 4.
	* Snoopy. Snoopy is a PHP class that makes it MUCH easier to submit forms (like MediaWiki edit forms, for example) via PHP.
		...
		You should probably take a quick look at Snoopy's readme before trying to understand my code.
...
likely to have problems with my link harvesting functions, since some of them require classes that my wiki's custom template uses but your wiki probably doesn't use.
...
define('ALTPREFIX','/w'); // no trailing slash. The prefix on valid links that visitors usually see.
...
// You can find userid at Special:Preferences. Can (should) override before including this file.
// Use BOTDEV as username when developing a bot. For running bots, either create a new username or user SUPERBOT (see litboxBot.php for superbot credentials)
if (!defined('USERID')){	define('USERID','1');}
...
class BasicBot extends Snoopy{
	...
	****************************************/
	...
	function wikiFilter($title,$callback,$summary='',$callbackParams=array()){
		...
		$this->maxredirs = 0; // we don't want to redirect, or else we won't be able to sniff response codes to check for success.
	...
	function wikiConnect(){
		...
		if ($this->wikiConnected) 		// no need to repeat all this if it's already been done.
			return true;
		if (file_exists($this->wikiCookies) && (filemtime($this->wikiCookies) > (time() - COOKIETIME)) ){ // cache cookies for only 1 hour.
	...
	function wikiFilterAll($source,$callback,$summary='',$callbackParams=array(),$metaReload=DELAY,$stripCats=true){
		// don't change these next four lines; SpecialFilterAll() assumes they'll look just like this.
		if (!$_GET['cache'])	// the <meta> reload will append ?cache= to the current URL. If it's there, then we know we've already harvested the links. If not, start by harvesting.
			...
			$cache = $_GET['cache'];
		// don't change the preceding four lines.
		...
		if ($this->wikiFilter($link, $callback, $summary, $callbackParams)){
			...
			if (0==count($links)){	// if TRUE, then we're all done.
				...
				echo('All done!');
				if (''!=$_GET['nuFile']) // used by one of my bots (the RecentChangesBot). You can delete this.
		...
		if (1<count($_GET)){	// some form-based bots create additional GET vars that we need to preserve (besides just "cache", which we've already got)
		...
		if ($success){
			...
			$out .= '<p style="color:#f00;"><strong>Filtering failed.</strong> Will wait '.$metaReload.' seconds until the next attempt instead of the usual '.$usualReload.', just in case the failure happened due to moving too fast. You can obviously just hit "reload" if you want.</p>';
			$out .= '<p>If status code below if "200," then we probably encountered an editing error from the wiki. Append "&showall=1" to this page\'s URL, then reload, to see more information.</p>';
		}
	...
	****************************************/
	...
	function wikiLogin(){
		...
		if ($this->submit($loginUrl,$vars)){
			/* 	okay, our 4 cookies will be now be in $this->cookies as an array. They look something like this:
			...
			*/
			if (is_array($this->cookies)){
				$cookiesCache = '<?php $cookiesCache = array( ';
	...
	function wikiHarvestLinks($source,$stripCats=true){
		//if (!$this->wikiConnect())	// uncomment these two lines if logging in is required to read the page you're trying to scrape.
		//	die( "Unable to connect." );
		if (!$this->fetch( $this->wikiServer . PREFIX . '/index.php?title=' . $title . '&action=raw' ))
			return false;
	...
	// like wikiHarvestLinks, stores the harvested links to cache and returns the cache filename.
	function HarvestSpecialLinks($title){
		...
		// Special pages use relative links. E.g. if ALTPREFIX is '/w', then you'll have things like '/w/Article_Title' in the special page's links.
	...
	// USED BY OTHER BOTS, SO DON'T MODIFY. (e.g. used by RecentChangesBot)
	function wikiLinks($content,$stripCats=false){	// set $stripCats to TRUE if you don't want categories returned.
		...
		*/
		...
		// make sure we replace all spaces with underscores in link targets.
		foreach( $internal as $key=>$link ){
			$internal[$key][1] = str_replace(' ', '_', $link[1]); // don't change. Used by recentchangesBot
		}
	...
	// you don't really need to call this, though; it gets call by ArrayFilterAll() automatically.
...
function inString($haystack,$needle,$insensitive=false){
...
function addCategory($content,$args){
	...
	elseif (inString($content, '[[Category:'))
		$content .= " $cat"; // if there are already cats, they're probably at the end. Let's just tack one one.
	else
...
// A sample callback for use with wikiFilter(). This is completely useless other than to demonstrate adding and removing categories.
// It will toggle whether a page has the "robots" category:
function dumbFilter($content){
	if (inString($content,'[[Category:Robots]]'))		// if the category doesn't exist already...
		$content = str_replace( '[[Category:Robots]]', '', $content); // add the category
	else 				// if the summary already has the category...
		$content .= ' [[Category:Robots]]';	// remove the category
...
/* 	SAMPLE USAGE
	...
	DEMO 3: Applying a filter to a whole bunch of articles is just as easy.
		Run the content of all articles linked to by 'Category:Test_Pages' through dumbFilter(), leaving edit summary 'Testing a bot on lots of pages' on each affected article:
			$myBot->wikiFilterAll( 'Category:Test_Pages', 'dumbFilter', 'Testing a bot on lots of pages' );
	...
	DEMO 4: It's just as easy if we're using a callback that accepts parameters. Let's repeat the preceding example, but now we'll use dumbFilter2 and pass $newtext='Wuzzup?'
		$myBot->wikiFilterAll( 'Category:Test_Pages', 'dumbFilter2', 'Testing a bot on lots of pages', array('newtext'=>'Wuzzup?') );
