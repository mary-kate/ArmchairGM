...
class WikiaSearch extends SpecialPage {
	...
	function WikiaSearch() {
		...
		{
			$this->goResult( $search = $wgRequest->getText( 'search') );
		}
		...
		$wgMessageCache->addMessage('noresults', 'There are no results');
	}
	...
	function getLuceneResultsBox($results, $title = '', $page = 'page', $showcontent = false) {
		global $luceneresulttemplate, $resultsnum, $noresults;
		...
		if($results['responseHeader']['numFound'] == 0) {
			return sprintf($resultsnum, $title, '') . sprintf($noresults, wfMsg('noresults'));
		}
		...
		for($i = 0; $i < count($results['results']); $i++) {
			...
			$html .= sprintf($luceneresulttemplate,  $res['url'], $res['title'], $fragment, htmlspecialchars($res['url']), $res['url'], wfMsg('length', $res['size']) , date('j M Y', wfTimestamp(TS_UNIX, $res['date'])));
		}
