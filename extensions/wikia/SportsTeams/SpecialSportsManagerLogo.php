...
$wgExtensionFunctions[] = 'wfSpecialSportsManagerLogo';

function wfSpecialSportsManagerLogo(){
  global $wgUser,$IP;
  include_once("includes/SpecialPage.php");


class SportsManagerLogo extends SpecialPage {
  ...
  function SportsManagerLogo(){
    UnlistedSpecialPage::UnlistedSpecialPage("SportsManagerLogo");
  }

  function execute(){
	  global $wgRequest, $IP;
				
	  $this->team_id = 		$wgRequest->getVal("id");
  ...
  function initLogo(&$request){
	...
	function executeLogo() {
		...
		$this->avatarUploadDirectory = $wgUploadDirectory . "/sport_logos"; 
...
function createThumbnail($imageSrc, $ext,$imgDest,$thumbWidth){
	...
	function saveUploadedFile( $saveName, $tempName, $ext) {
		...
		if($ext == "JPG" && is_file( $this->avatarUploadDirectory . "/" . $this->team_id . "_l.jpg")){$type = 2;}
		if($ext == "GIF" && is_file( $this->avatarUploadDirectory . "/" . $this->team_id. "_l.gif")){$type = 1;}
		if($ext == "PNG" && is_file( $this->avatarUploadDirectory . "/" . $this->team_id . "_l.png")){$type = 3;}
		
		if($ext!="JPG"){
			 if(is_file($this->avatarUploadDirectory . "/" . $this->team_id . "_s.jpg") ) unlink($this->avatarUploadDirectory . "/" . $this->team_id . "_s.jpg");
			 if(is_file($this->avatarUploadDirectory . "/" . $this->team_id . "_m.jpg") ) unlink($this->avatarUploadDirectory . "/" . $this->team_id . "_m.jpg");
			 if(is_file($this->avatarUploadDirectory . "/" . $this->team_id . "_l.jpg") ) unlink($this->avatarUploadDirectory . "/" . $this->team_id . "_l.jpg");	
		}
		if($ext!="GIF"){
			 if(is_file($this->avatarUploadDirectory . "/" . $this->team_id . "_s.gif") ) unlink($this->avatarUploadDirectory . "/" . $this->team_id . "_s.gif");
			 if(is_file($this->avatarUploadDirectory . "/" . $this->team_id . "_m.gif") ) unlink($this->avatarUploadDirectory . "/" . $this->team_id . "_m.gif");
			 if(is_file($this->avatarUploadDirectory . "/" . $this->team_id . "_l.gif") ) unlink($this->avatarUploadDirectory . "/" . $this->team_id . "_l.gif");	
		}
		if($ext!="PNG"){
			 if(is_file($this->avatarUploadDirectory . "/" . $this->team_id . "_s.png") ) unlink($this->avatarUploadDirectory . "/" . $this->team_id . "_s.png");
			 if(is_file($this->avatarUploadDirectory . "/" . $this->team_id . "_m.png") ) unlink($this->avatarUploadDirectory . "/" . $this->team_id . "_m.png");
			 if(is_file($this->avatarUploadDirectory . "/" . $this->team_id . "_l.png") ) unlink($this->avatarUploadDirectory . "/" . $this->team_id . "_l.png");	
		}
	...
	function showSuccess($status) {
		...
		$output = "<h2>" . wfMsgForContent( 'st_logo_upload_success' ) . "</h2>";
		$output .= "<h5>" . wfMsgForContent( 'st_logo_images_below' ) . "</h5>";
		if($status==1)$ext = "gif";
		if($status==2)$ext = "jpg";
		if($status==3)$ext = "png";
		
		$output .= "<table cellspacing=0 cellpadding=5>";
		$output .= "<tr><td valign=top style='color:#666666;font-weight:800'>" . wfMsgForContent( 'st_logo_size_large' ) . "</td><td><img src=images/sport_logos/" . $this->team_id . "_l." . $ext . "?ts=" .  rand()   . "></td></tr>";
		$output .= "<tr><td valign=top style='color:#666666;font-weight:800'>" . wfMsgForContent( 'st_logo_size_medium' ) . "</td><td><img src=images/sport_logos/" . $this->team_id . "_m." . $ext . "?ts=" .  rand()   . "></td></tr>";
		$output .= "<tr><td valign=top style='color:#666666;font-weight:800'>" . wfMsgForContent( 'st_logo_size_small' ) . "</td><td><img src=images/sport_logos/" . $this->team_id . "_s." . $ext . "?ts" .  rand()   . "=></td></tr>";
		$output .= "<tr><td><input type=button onclick=javascript:history.go(-1) value='" . wfMsgForContent( 'st_logo_go_back' ) . "'></td></tr>";
		
		$output .= "<tr><td><a href=\"". Title::MakeTitle(NS_SPECIAL, "SportsTeamsManager")->escapeFullUrl() ."\">" . wfMsgForContent( 'st_logo_back_to_list' ) . "</a> |";
		$output .= "  </td></tr>";
		$output .= "</table>";
	...
	function uploadError( $error ) {
		...
		$wgOut->addHTML("<br><input type=button class=\"site-button\" onclick=javascript:history.go(-1) value='" . wfMsgForContent( 'st_logo_go_back' ) . "'>");
	}
	...
	<table border='0'>
		<tr>
			...
			<tr>
				<td align='right'>
					<input tabindex='2' type='button' onclick=javascript:history.go(-1) value='" . wfMsgForContent( 'st_logo_back' ) . "' />
				</td>
	...
	" ;
		...
		if($team_logo != ""){
			$output = "<table><tr><td style='color:#666666;font-weight:800'>" . wfMsgForContent( 'st_logo_current_image' ) . "</td></tr>";
			$output .= "<tr><td><img src=\"images/sport_logos/" . $team_logo . "\" border=\"0\" alt=\"no logo\" /></td></tr></table><br>";
		}
		$wgOut->addHTML($output);
	...
	<td style='color:#666666;font-weight:800'>" . wfMsgForContent( 'st_logo_image_instructions' ) . "<br>
	...
	*/
	function detectVirus($file) {
		...
		if (!$wgAntivirusSetup[$wgAntivirus]) { 
			...
			$wgOut->addHTML( "<div class='error'>" . wfMsgForContent( 'st_logo_virus_scan_error', $wgAntivirus ) . "</div>\n" ); #LOCALIZE
 ...
 SpecialPage::addPage( new SportsManagerLogo );
