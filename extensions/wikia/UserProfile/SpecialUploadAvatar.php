...
function createThumbnail($imageSrc, $ext,$imgDest,$thumbWidth){
	...
	function saveUploadedFile( $saveName, $tempName, $ext) {
		...
		global $wgMemc;
		$key = wfMemcKey( 'user', 'profile', 'avatar-v2', $wgUser->getID(), "s" );
		$data = $wgMemc->delete( $key );
		
		$key = wfMemcKey( 'user', 'profile', 'avatar-v2', $wgUser->getID(), "m");
		$data = $wgMemc->delete( $key );
		
		$key = wfMemcKey( 'user', 'profile', 'avatar-v2', $wgUser->getID() , "l");
		$data = $wgMemc->delete( $key );
		
		$key = wfMemcKey( 'user', 'profile', 'avatar-v2', $wgUser->getID() , "ml");
		$data = $wgMemc->delete( $key );
		if($type > 0 ){
