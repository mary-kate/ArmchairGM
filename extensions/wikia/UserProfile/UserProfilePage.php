...
class UserProfilePage extends Article{
	...
	function view(){
		...
		if ( !$this->user_id ) {
			$wgOut->addHTML( "User does not exist?" );
		} 
	...
	*/
	function getMiniGallery($user_name){
		...
		if ($picture_count>0) {
			...
			<div class=\"picture-container clearfix\">";
					...
					foreach( $pictures["list"] as $picture ){
						...
						if( is_object( $image ) ){
							$thumb = $image->getThumbnail(75,75,true);
							$thumbnail = $thumb->toHtml();
						}

						$image_link = Title::MakeTitle(NS_IMAGE, $image_path);

						$output .= "<div id=\"mini-gallery-" . ($x-1) . "\" class=\"mini-image\"><a href=\"".$image_link->escapeFullURL()."\">{$thumbnail}</a></div>";
		...
		$output .= "<div id=\"upload-frame-errors\" style=\"display:none;\" class=\"upload-frame-errors\"></div>
		<div id=\"upload-container\" style=\"display:none;\" class=\"upload-container\">
			<iframe id=\"imageUpload-frame\" class=\"imageUpload-frame\" width=\"410\" 
				scrolling=\"no\" frameborder=\"0\" src=\"".$upload_picture_title->escapeFullURL('wpThumbWidth=75')."\">
			</iframe>
		</div>
		<div class=\"picture-container-spacer\"></div>";
		return $output;
	}
					...
					if ($item["type"]=="Picture Game") {
						if( $item["img1"] != "" && $item["img2"] != ""){
							...
							if( is_object( $render_1 ) ){
								$thumb_1 = $render_1->getThumbNail(25,0,true);
								if( is_object( $thumb_1 ) ) $image_1 = $thumb_1->toHtml();
							}
	
							$render_2 = Image::newFromName ($item["img2"]);
							if( is_object( $render_2 ) ){
								$thumb_2 = $render_2->getThumbNail(25,0,true);
								if( is_object( $thumb_2 ) ) $image_2 = $thumb_2->toHtml();
							}
	...
	function getProfileTop($user_id, $user_name) {
		...
		if ( $this->isOwner() ) {
			...
			$output .= "<a href=\"".$send_message->escapeFullURL('user='.$wgUser->getName().'&conv='.$user_safe)."\" rel=\"nofollow\">".wfMsg("user-send-message")."</a> | <a href=\"".$give_gift->escapeFullURL('user='.$user_safe)."\" rel=\"nofollow\">".wfMsg("user-send-gift")."</a> |";
	...
	function getFanBoxes($user_name){
		global $wgOut, $IP, $wgUser, $wgTitle, $wgMemc, $wgUserProfileDisplay;
		...
		$fanbox_link = Title::Maketitle(NS_SPECIAL, "ViewFanBoxes");
		$per_row = 1;
		...
		if ($fanboxes) {
			...
			<div class=\"cleared\"></div>
			<div class=\"user-fanbox-container\">";
				...
				foreach ($fanboxes as $fanbox) {
					...
					<div class=\"show-message-container\" id=\"show-message-container".$fanbox["fantag_id"]."\">
						<div class=\"permalink-containerhalf\">
						...
						<table  class=\"fanBoxTableHalf\" onclick=\"javascript:openFanBoxPopup('fanboxPopUpBox{$fanbox["fantag_id"]}', 'individualFanbox{$fanbox["fantag_id"]}')\" border=\"0\" cellpadding=\"0\" cellspacing=\"0\" >
						<tr><td id=\"fanBoxLeftSideOutput\" style=\"color:".$fanbox["fantag_left_textcolor"]."; font-size:$leftfontsize\" bgcolor=\"".$fanbox["fantag_left_bgcolor"]."\">".$fantag_leftside."</td> 
						<td id=\"fanBoxRightSideOutput\" style=\"color:".$fanbox["fantag_right_textcolor"]."; font-size:$rightfontsize\" bgcolor=\"".$fanbox["fantag_right_bgcolor"]."\">".$fanbox["fantag_right_text"]."</td>
						</table>
						</div>
					</div>
					</div>";
					...
					if($wgUser->isLoggedIn()){
						if ($check_user_fanbox == 0){
							$output .= "
						<div class=\"fanboxpopup-container\">
						<div class=\"fanbox-pop-up-boxhalf\" id=\"fanboxPopUpBox".$fanbox["fantag_id"]."\">
						<table class=\"popupBoxHalf\" cellpadding=\"0\" cellspacing=\"0\" ><tr><td id=\"popupBoxContents\">". wfMsgForContent( 'fanbox_add_fanbox' ) ."<tr><td id=\"popupBoxButtons\">
						<input type=\"button\" value=\"Add\" size=\"20\" onclick=\"closeFanboxAdd('fanboxPopUpBox{$fanbox["fantag_id"]}', 'individualFanbox{$fanbox["fantag_id"]}'); showAddRemoveMessageUserPageHalf(1, {$fanbox["fantag_id"]})\" />
						<input type=\"button\" value=\"Cancel\" size=\"20\" onclick=\"closeFanboxAdd('fanboxPopUpBox{$fanbox["fantag_id"]}', 'individualFanbox{$fanbox["fantag_id"]}')\" />
						</td></table>
						</div>
						</div>";
						}
						...
						<div class=\"fanboxpopup-container\">
						<div class=\"fanbox-pop-up-boxhalf\" id=\"fanboxPopUpBox".$fanbox["fantag_id"]."\">
						<table class=\"popupBoxHalf\" cellpadding=\"0\" cellspacing=\"0\" ><tr><td id=\"popupBoxContents\">". wfMsgForContent( 'fanbox_remove_fanbox' ) ."<tr><td id=\"popupBoxButtons\">
						<input type=\"button\" value=\"Remove\" size=\"20\" onclick=\"closeFanboxAdd('fanboxPopUpBox{$fanbox["fantag_id"]}', 'individualFanbox{$fanbox["fantag_id"]}'); showAddRemoveMessageUserPageHalf(2, {$fanbox["fantag_id"]})\" />
						<input type=\"button\" value=\"Cancel\" size=\"20\" onclick=\"closeFanboxAdd('fanboxPopUpBox{$fanbox["fantag_id"]}', 'individualFanbox{$fanbox["fantag_id"]}')\" />
						</td></table>
						</div>
						</div>";
					...
					if($wgUser->getID() == 0 ){
						...
						$output .= "<div class=\"fanboxpopup-container\"><div class=\"fanbox-pop-up-boxhalf\" id=\"fanboxPopUpBox".$fanbox["fantag_id"]."\">
						<table class=\"popupBoxHalf\" cellpadding=\"0\" cellspacing=\"0\" ><tr><td id=\"popupBoxContents\">". wfMsgForContent( 'fanbox_add_fanbox_login' ) ."<a href=\"{$login->getFullURL()}\">". wfMsgForContent( 'fanbox_login' ) ."</a><tr><td id=\"popupBoxButtons\">
						<input type=\"button\" value=\"Cancel\" size=\"20\" onclick=\"closeFanboxAdd('fanboxPopUpBox{$fanbox["fantag_id"]}', 'individualFanbox{$fanbox["fantag_id"]}')\" />
						</td></table>
						</div></div>";
					};
	...
	function getUserBoard($user_id,$user_name){
		global $IP, $wgMemc, $wgUser, $wgTitle, $wgOut, $wgUserProfileDisplay;
		if($user_id == 0)return "";
		...
		require_once("$IP/extensions/wikia/UserRelationship/UserRelationshipClass.php");
		$wgOut->addScript("<script type=\"text/javascript\" src=\"/extensions/wikia/UserProfile/UserProfilePage.js\"></script>\n");
