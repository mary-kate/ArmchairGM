...
$wgExtensionFunctions[] = 'wfSpecialNFLPlayerStats';

$team_translations = array();


function wfSpecialNFLPlayerStats(){
  global $wgUser,$IP;
  include_once("includes/SpecialPage.php");


class NFLPlayerStats extends SpecialPage {
	
	
	function NFLPlayerStats(){
		UnlistedSpecialPage::UnlistedSpecialPage("NFLPlayerStats");
	}
	
	function execute($value){
		global $wgRequest, $wgOut, $wgStyleVersion, $wgUser, $wgParser;
		if (!$value || strlen($value) != 1) {
			$this->executeSingleSearch($value);
		}
		else {
			$this->executeLargeSearch($value);
		}

	}
	
	function executeLargeSearch($value) {
		...
		if ($value && strlen($value) == 1) {
			...
			$sql = "SELECT player_name, count(player_name) as numbers, firstname, lastname, player_id FROM nfl_players_info WHERE lastname like '{$value}%' group by player_name order by lastname ASC, firstname ASC";
			...
			while($row = mysql_fetch_array($result)) {
				...
				if (intval($numbers) == 1) {
					...
					if( $do_edit){
						$article_text = $this->getNFLPlayerStats($row["firstname"], $row["lastname"], $row["player_id"], true);
						$article = new Article($title);
						//$article->doEdit( $article_text, "MLB Player Pages", EDIT_SUPPRESS_RC);
						$article->doEdit( $article_text["wiki"], "NFL Player Pages");
			...
			$title = "NFL Stats Bot player generator report for {$value}";
			...
			$summary_article->doEdit($summary_article_text, "NFL Stat Bot Reports");
	...
	function executeSingleSearch($value){
		global $wgRequest, $wgOut, $wgStyleVersion, $wgUser, $wgParser;
			
			
		$output = "<form name=\"form1\">
			<input type=\"hidden\" name=\"title\" id=\"title\" value=\"Special:NFLPlayerStats\" />
		...
		$title = "Search for an NFL Player Stats";
				
		if ($wgRequest->getVal("first") != "" || $wgRequest->getVal("last") != "") {
			$output_array = $this->getNFLPlayerStats($wgRequest->getVal("first"), $wgRequest->getVal("last"), false, false);
	
		}
		elseif ($wgRequest->getVal("playerid") != ""){
			$output_array = $this->getNFLPlayerStats(false, false, $wgRequest->getVal("playerid"), false);
		}
		else {
		}
		...
		if ($output_array["wiki"]) {
			...
			$total_matches = preg_match_all($re_pattern, $output_array["wiki"], $matches);
			//$categories = "";
	...
	function getNFLPlayerStats($first_name, $last_name, $player_id, $do_create_categories) {
		global $wgRequest;

		if (!$player_id) {
			$sql = "SELECT nfl_players_info.*, sum(team) as pro_experience from nfl_players_info JOIN nfl_fumbles_stats ON nfl_players_info.player_id=nfl_fumbles_stats.player_id WHERE (firstname LIKE '{$first_name}%' AND lastname LIKE '{$last_name}%') OR (player_fullname LIKE '{$first_name}%' AND player_fullname LIKE '% {$last_name}%') AND nfl_fumbles_stats.is_total=1 GROUP BY nfl_players_info.player_id ORDER BY lastname ASC, firstname ASC";
		}
		else {
...
SpecialPage::addPage( new NFLPlayerStats );
