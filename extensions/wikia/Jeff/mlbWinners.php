<?php
$wgExtensionFunctions[] = "wfMlbWinners";

function wfMlbWinners() {
    global $wgParser, $wgOut;
    $wgParser->setHook( "mlbWinners", "getWinners" );
}

function getWinners($input, $args, $parser) {
	...
	$contents = stream_get_contents($handle);
		...
		$result_info = result_exists($games[$i]["gameIdentifier"]);
		$transaction_result = "";
		
		if ($result_info["game_id"] >=0 && !$result_info["was_found"]) {
			do_result_insert($result_info["game_id"],$games[$i]["visTeamScore"],$games[$i]["homeTeamScore"],$games[$i]["winner"],$games[$i]["winnerAbbr"],$games[$i]["statusCode"],$games[$i]["gameStatus"],$sport_id,$games[$i]["gameIdentifier"]);
			...
			do_result_update($result_info["result_id"], $result_info["game_id"],$games[$i]["visTeamScore"],$games[$i]["homeTeamScore"],$games[$i]["winner"],$games[$i]["winnerAbbr"],$games[$i]["statusCode"],$games[$i]["gameStatus"],$sport_id,$games[$i]["gameIdentifier"]);
...
function do_result_insert($game_id,$vis_score,$home_score,$winner,$winner_abbr,$status,$status_desc,$sport_id,$identifier) {
	global $wgUser;
...
function do_result_update($result_id, $game_id,$vis_score,$home_score,$winner,$winner_abbr,$status,$status_desc,$sport_id,$identifier) {
	global $wgUser;
	$dbr =& wfGetDB( DB_MASTER );
...
function result_exists($pick_identifier) {
