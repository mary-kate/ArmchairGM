...
$wgExtensionFunctions[] = 'wfSpecialMiniAjaxUpload';

function wfSpecialMiniAjaxUpload(){
	...
	class MiniAjaxUpload extends SpecialPage {
		...
		function execute(){
			...
			$form = new MiniAjaxUploadForm($wgRequest);
			$form->execute();
		}
...
class MiniAjaxUploadForm extends UploadForm{
	...
	function mainUploadForm( $msg='' ) {
		...
		//'
		$wgOut->addHTML( "
			<script language=\"javascript\" src=\"/extensions/Prototype/prototype.js\"></script>
			<script language=\"javascript\">
			
			function submitForm(){
				
				if(Form.Element.getValue('wpUploadFile') != ''){
			...
			<form id='upload' name='upload' onSubmit=\"return submitForm()\" method='post' enctype='multipart/form-data' action=\"$action\">
				...
				<tr>
					<td>
					<input tabindex='9'  type='Submit' class='startButton' id='wpUpload' name='wpUpload' value=\"Upload\" />
					</td>
				</tr>
				</table>
			</form>
	...
	//<tr>
		...
		//<td>
			//<label for='wpUploadFile'><b>{$sourcefilename}</b></label>
		//</td>
	//</tr>
	
	//<tr>
		//<td>
		//<label for='wpUploadDescription'><b>{$summary}</b></label>
		//</td>
	//</tr>
	//<tr>
		//<td>
		//<input tabindex='2' type='". ($isOverwrite ? "hidden" : "text") . "' 
		//name='wpUploadDescription' id='wpUploadDescription' size='40' 
		//value=\"" . htmlspecialchars( $this->mUploadDescription ) ."\" />
	...
	function showSuccess() {
		...
		if(strlen($inserCat) == 0) 
			$cat = "Profilepictures";
		else
			$cat = addslashes ( $insertCat );
		...
		$thumb = $img->getThumbnail( $thumbWidth, 0, true );
		$img_tag = $thumb->toHtml();
		
		
		// There HAS to be a better way to get the image description
		...
		$article->doEdit(  "{{Image Top}}\n\n{{Image Bottom}}\n[[Category:Profile Pictures]]\n[[Category:Profile Pictures by User {$user_name}]]", "aprofile image creation" );
