...
$wgAjaxExportList [] = 'wfGetWidgetText';
...
function wfGetWidgetText() {
	...
	$w_text_stripped = doFlashFormatting($w_text_parsed);
...
function getUsableContent($page) {
...
function doFlashFormatting($text) {
	
	$text = removeAllContentFromTags($text);
	$text = changeFormattingForFlash($text);
	$text = removeUnallowedTags($text);
	$text = removeEmptySections($text);
	$text = setDefaultParams($text);
	$text = removeLinkFromImagesAndResize($text);
	$text = updateEditLinks($text);
	$text = resolveRelativeLinks($text);
...
function removeUnallowedTags($text) {
	$allowed_tags = array(
		'a'=>array('href','target'),
		'b',
		'br',
...
function changeFormattingForFlash($text) {
	$replace_tags = array(
...
function setDefaultParams($text) {
...
function removeLinkFromImagesAndResize($text) {
...
function removeAllContentFromTags($text) {
	$remove_tags = array(
		'script',
		//'table'
		);
