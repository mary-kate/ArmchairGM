...
global $wgNamespaceTemplates, $wgHooks, $wgExtensionsFunctions;
...
$wgExtensionCredits['other'][] = array(
    ...
    'description' => "allows for image tagging"
);
...
function wfCheckArticleImageTags($outputPage, $text)
{
    ...
    if ( $outputPage->isArticle() )
    {
        ...
        $res = $db->query("SELECT article_tag, tag_rect, unique_id, COUNT(article_tag) AS count FROM ".
			" `$wgDBname`.`imagetags` ".
			" WHERE article_tag='" . $wgTitle->getText(). "'" .  /*LIMIT 1"*/          " GROUP BY article_tag" );
...
function addTag($action, $article)
{
    ...
    if ($img)
    {
       ...
       $logComment = "Image '$imgName' was tagged to article \"$tagName\" at location $tagRect by $userText";
...
function removeTag($action, $article)
{
    ...
    if ($img)
    {
        ...
        $logPage = new LogPage( 'tag' );
        $logComment = "Removed tag to article \"$tagName\" at location $tagRect by $userText";
       $logPage->addEntry( 'tag', $imgTitle, $logComment);
...
function wfGetImageTags($img, $imgName) {
    ...
    $res = $db->query("SELECT article_tag, tag_rect, unique_id FROM ".
          " `$wgDBname`.`imagetags` ".
	...
	$specialImagesTitle = Title::newFromText("Special:TaggedImages");
        ...
        $removeLink = '<a href="#" onclick="removeTag(' . $o->unique_id . ', this); return false;">' . wfMsgHtml('removetag') . '</a>';
...
class ImageTagPage extends ImagePage
{
    function openShowImage() {
        global $wgOut, $wgUser, $wgServer, $wgImageTaggingPath;
	...
	if ( $wgImageTaggingPath == '' ) {  $wgImageTaggingPath = '/wiki/extensions/wikia/ImageTagging/';  }
        ...
        $wgOut->addScript('<script type="text/javascript" src="' . $wgImageTaggingPath . 'img_tagging.js" charset="utf-8"></script>');
        $wgOut->addScript('<script type="text/javascript" src="' . $wgImageTaggingPath . 'json.js" charset="utf-8"></script>');
	...
	// $wgOut->addScript('<script type="text/javascript" src="' . $wgImageTaggingPath . 'WSuggest.js"></script>');
        ...
        if ( $wgUser->isAllowed('edit') && 
             ...
             $this->mTitle->isProtected('edit') == false )
