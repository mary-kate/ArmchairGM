...
class TaggedImages extends SpecialPage
{
	...
	$wgOut->addScript("<style type=\"text/css\" >@import \"/skins/common/img_tagging.css\"></style>");

        $this->mQuery = $wgRequest->getText('q');
        $this->mStartPage = $wgRequest->getVal('page'); 
        $this->mCount = 0;
	...
	if (! $this->mQuery) {
	  $wgOut->addHTML("<h3>Error: no query provided</h3>");
	  return;
	}
        # COUNT(img_name) AS img_count
        $res = $db->query("SELECT imagetags.img_name, image.img_timestamp FROM " .
		      " `$wgDBname`.`imagetags` LEFT JOIN `$wgDBname`.`image` ON imagetags.img_name = image.img_name ".
		      " WHERE article_tag='" . $this->mQuery . "'" .
		      " GROUP BY unique_id " . 
	...
	$wgOut->setPageTitle( wfMsg('taggedimages_title', $this->mQuery) );
