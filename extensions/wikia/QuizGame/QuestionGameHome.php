...
function wfSpecialQuizGameHome(){
	...
	class QuizGameHome extends SpecialPage {
		...
		function adminPanel(){
			...
			$sql = "SELECT q_id, q_text, q_flag, q_picture FROM quizgame_questions WHERE q_flag=\"FLAGGED\" OR q_flag=\"PROTECT\";";
			$res = $dbr->query($sql);
			//"
			$flaggedQuestions = "";
			$protectedQuestions = "";
			while ( $row = $dbr->fetchObject( $res ) ) {
				...
				$buttons = "<a href=\"index.php?title=Special:QuizGameHome&questionGameAction=editItem&quizGameId={$row->q_id }&quizGameKey={$key}\">" . wfMsgForContent( 'quiz_edit' ) . "</a> -
			...
			$output .= "<div class=\"quizgame-admin\" id=\"quizgame-admin\">
					...
					<div class=\"quizgame-admin-top-links\">
						<a href=\"index.php?title=Special:QuizGameHome&questionGameAction=launchGame\">" . wfMsgForContent( 'quiz_admin_back' ) . "</a>
					</div>
		...
		function completeEdit(){
			...
			header("Location: index.php?title=Special:QuizGameHome&questionGameAction=renderPermalink&permalinkID={$id}");
		...
		function renderLoginPage(){
			...
			$output .= "<div>
				<input type=\"button\" class=\"site-button\" value=\"" . wfMsgForContent( 'quiz_main_page_button' ) . "\" onclick=\"window.location='index.php?title=Main_Page'\"/> 
				<input type=\"button\" class=\"site-button\" value=\"" . wfMsgForContent( 'quiz_login_button' ) . "\" onclick=\"window.location='index.php?title=Special:Login'\"/>
			</div>";
			$wgOut->addHTML($output);
		}
		...
		function renderQuizOver(){
			...
			$output = wfMsgForContent( 'quiz_ohnoes' );
			$output .= "<div>
		...
		function renderPermalinkError(){
			...
			$output = wfMsgForContent( 'quiz_unavailable' );
			$wgOut->addHTML($output);	
		}
		...
		function renderStart(){
			...
			$output = wfMsgForContent( 'quiz_intro' ) . "  <a href=\"index.php?title=Special:QuizGameHome&questionGameAction=launchGame\">" . wfMsgForContent( 'quiz_introlink' ) . "</a>";
			$wgOut->addHTML($output);	
		}
		...
		function launchGame(){
			global $wgRequest, $wgUser, $wgOut, $wgRequest, $wgSiteView, $IP, $wgUploadPath, $wgSiteName;
			...
			$wgOut->setOnloadHandler( $on_load );
			...
			if (($wgUser->getID() == $question["user_id"] || ($user_answer && $wgUser->isLoggedIn() && $wgUser->isAllowed("delete")) || in_array('staff',($wgUser->getGroups()))) && ($wgUseEditButtonFloat == true)) {
				$editMenu = "
				...
				$editLinks = "
				    <a href=\"index.php?title=Special:QuizGameHome&questionGameAction=adminPanel\">" . wfMsgForContent( 'quiz_admin_panel_title' ) . "</a> -
			...
			if($wgUser->isLoggedIn()){
				...
				$stats_box = "<div class=\"user-rank\"><h2>" . wfMsgForContent( 'quiz_leaderboard_scoretitle' ) . "</h2><a href=\"index.php?title=Special:Login\">" . wfMsgForContent( 'quiz_login' ) . "</a> " . wfMsgForContent( 'quiz_or' ) . " <a href=\"index.php?title=Special:UserRegister\">" . wfMsgForContent( 'quiz_create_account' ) . "</a> " . wfMsgForContent( 'quiz_leaderboard_climb' ) . "</div>";
			}
			...
			<script type=\"text/javascript\" src=\"{$this->INCLUDEPATH}launchgame.js\"></script>
			...
			";
					...
					if( !$user_answer && $wgUser->getName() != $question["user_name"] ){
						...
						if(!$user_answer && $wgUser->getName() != $question["user_name"] ){
							...
							$output .=  "<a href=\"index.php?title=Special:QuizGameHome&questionGameAction=launchGame\">" . wfMsgForContent( 'quiz_next' ) . "</a>";
						}
						$output .= "</div>";
							...
							if($prev_question["id"] && $prev_question["user_answer"]) {
								...
								<div class=\"last-question-heading\">
									" . wfMsgForContent( 'quiz_last_question' ) . " - <a href=\"index.php?title=Special:QuizGameHome&questionGameAction=renderPermalink&permalinkID={$prev_question["id"]}\">{$prev_question["text"]}</a>
								...
								if($choice["id"] == $prev_question["user_answer"]){
									...
									if($choice["is_correct"] == 1){
										$your_answer_status = "<div class=\"answer-status-correct\">
											" . wfMsgForContent( 'quiz_chose_incorrect', $prev_question["points"] ) ."
										</div>";
									}else{
										$your_answer_status = "<div class=\"answer-status-incorrect\">
											" . wfMsgForContent( 'quiz_next' ) . "
										</div>";
									}
								}
							}
					...
					<div class=\"quizgame-right\">
						...
						<div class=\"create-link\"><img border=\"0\" src=\"{$wgUploadPath}/common/addIcon.gif\"/> <a href=\"index.php?title=Special:QuizGameHome&questionGameAction=createForm\">" . wfMsgForContent( 'quiz_create_title' ) . "</a></div>
						<div class=\"credit-box\" id=\"creditBox\">
							<h1>" . wfMsgForContent( 'quiz_submitted_by' ) . "</h1>
...
$admin_panel_link = Title::makeTitle(NS_SPECIAL,"QuizGameHome");
								...
								//{$editLinks}
								$output .= "<a href=\"javascript:document.location='index.php?title=Special:QuizGameHome&questionGameAction=renderPermalink&permalinkID=' + \$( 'quizGameId' ).value\">" . wfMsgForContent( 'quiz_permalink' ) . "</a>
							</div>
					</div>
				</div>
		...
		function createQuizGame(){
			...
			if($key != md5( $this->SALT . $chain ) ){
				header( 'Location: ?title=Special:QuizGameHome' ) ;
				return;
			}
			...
			$message = wfMsgForContent( 'quizquestionslog-create-text', "Special:QuizGameHome/{$question_id}" );
			...
			header( 'Location: ?title=Special:QuizGameHome&questionGameAction=renderPermalink&permalinkID=' . $question_id );
		...
		function renderWelcomePage(){
			...
			$output .= "<div id=\"quiz-container\" class=\"quiz-container\">
				...
				<div class=\"create-message\">
					...
					<p>" . wfMsgForContent( 'quiz_create_message' ) . "</p>
					<p><input class=\"site-button\" type=\"button\" onclick=\"document.location='index.php?title=Special:QuizGameHome&questionGameAction=launchGame'\" value=\"" . wfMsgForContent( 'quiz_play_quiz' ) . "\"/></p>
				</div>
				
				<div class=\"quizgame-create-form\" id=\"quizgame-create-form\">		
					<form id=\"quizGameCreate\" name=\"quizGameCreate\" method=\"post\" action=\"index.php?title=Special:QuizGameHome&questionGameAction=createGame\">
				...
				<div id=\"quizgame-picture-upload\" style=\"display:block;\">
					...
					<div id=\"real-form\" style=\"display:block;height:70px;\">
						<iframe id=\"imageUpload-frame\" class=\"imageUpload-frame\" width=\"420\" 
							scrolling=\"no\" border=\"0\" frameborder=\"0\" src=\"index.php?title=Special:QuestionGameUpload&wpThumbWidth=75&wpCategory=Quizgames\">
						</iframe>
					</div>
				</div>
