...
class InviteEmail extends UnlistedSpecialPage {
	...
	function execute(){
		...
		if($wgRequest->wasPosted() && $_SESSION["alreadysubmitted"] == false){
			...
			$out .= "<p>";
				$out .= "<input type=\"button\" class=\"invite-form-button\" value=\"" . wfMsgForContent( 'invite_more_friends' ) ."\" onclick=\"window.location='index.php?title=Special:InviteEmail'\"/> ";
				$out .= "<input type=\"button\" class=\"invite-form-button\" value=\"" . wfMsgForContent( 'invite_see_whos_online' ) ."\" onclick=\"window.location='index.php?title=Special:InviteContacts'\"/> ";
			$out .= "</p>";
	...
	function getInviteEmailContent($type){
		...
		switch ($type) {
			...
			default:
				...
				$email["subject"] = wfMsg( 'invite_subject',
							$user_label
							);
				
				$email["body"] = wfMsg( 'invite_body',
							$user_label,
							$user_label,
							$title->getFullURL(),
