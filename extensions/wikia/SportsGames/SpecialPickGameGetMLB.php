...
$wgExtensionFunctions[] = 'wfSpecialPickGameGetMLB';

function wfSpecialPickGameGetMLB(){
  global $wgUser,$IP;
  include_once("includes/SpecialPage.php");


class PickGameGetMLB extends SpecialPage {

	
	function PickGameGetMLB(){
		UnlistedSpecialPage::UnlistedSpecialPage("PickGameGetMLB");
	}
	...
	function execute($value){
		...
		while ($startpos > 0) {
		 ...
		 $startpos = strpos($contents, "<a href=\"/mlb/teams/page/");
		    ...
		    $visTeam = $this->MLBcheckLASwitch($visTeam);
		    ...
		    $homeTeam = $this->MLBcheckLASwitch($homeTeam);
		...
		for ($i=0; $i<sizeof($games); $i++) {
		    ...
		    */
		    ...
		    if (!$this->MLBgame_exists($games[$i]["gameIdentifier"])) {
			$this->do_MLBgame_insert($games[$i]["gameTime"]-$theOffset, $games[$i]["homeTeamName"], $games[$i]["visTeamName"], $games[$i]["gameIdentifier"], $games[$i]["homeTeam"], $games[$i]["visTeam"], $home_addl, $vis_addl, $games[$i]["game_date"], 1);
			$dbOutput .= "Inserted " . $games[$i]["gameIdentifier"] . "<br/>";
			    
		    }
		    else {
			    $this->do_MLBgame_update($games[$i]["gameTime"]-$theOffset, $games[$i]["homeTeamName"], $games[$i]["visTeamName"], $games[$i]["gameIdentifier"], $games[$i]["homeTeam"], $games[$i]["visTeam"], $home_addl, $vis_addl, $games[$i]["game_date"], 1);
			    $dbOutput .= "Updated " . $games[$i]["gameIdentifier"] . "<br/>";		    
		    }
		...
		$title = "Get MLB Starters for the next 10 days";
	...
	function MLBcheckLASwitch($abbr) {
	    if ($abbr == "LAD") {
		return "LA";
	    }
	    elseif ($abbr == "LAA") {
	...
	function MLBgame_exists($pick_identifier) {
	...
	function do_MLBgame_insert($date,$home,$vis,$identifier,$home_abbr,$vis_abbr,$home_addl,$vis_addl,$game_date,$sport_id) {
		global $wgUser;
		$dbr =& wfGetDB( DB_MASTER );
	...
	function do_MLBgame_update($date,$home,$vis,$identifier,$home_abbr,$vis_abbr,$home_addl,$vis_addl,$game_date,$sport_id) {
		global $wgUser;
		$dbr =& wfGetDB( DB_MASTER );
...
SpecialPage::addPage( new PickGameGetMLB );
