...
function wfSpecialChallengeStandings(){
	...
	class ChallengeStandings extends SpecialPage {
		...
		function ChallengeStandings(){
			UnlistedSpecialPage::UnlistedSpecialPage("ChallengeStandings");
		}
		
		function execute(){
			global $wgUser, $wgOut, $wgRequest;
			$wgOut->setPagetitle( "Challenge Standings" );
			$css = "<style>
			...
			$sql = "SELECT challenge_record_username, challenge_record_user_id,challenge_wins, challenge_losses, challenge_ties, (challenge_wins / (challenge_wins + challenge_losses + challenge_ties) ) as winning_percentage FROM challenge_user_record ORDER BY (challenge_wins / (challenge_wins + challenge_losses + challenge_ties) ) DESC, challenge_wins DESC LIMIT 0,25";
			$res = $dbr->query($sql);
			$x = 1;
