...
function wfSpecialPictureGameHome(){
	...
	class PictureGameHome extends SpecialPage {
		...
		function getMiniGame(){
			...
			$sql = "SELECT * FROM picturegame_images WHERE picturegame_images.id NOT IN (SELECT picid FROM picturegame_votes WHERE picturegame_votes.username='" . addslashes($wgUser->getName()) . "') AND flag != 'FLAGGED' and img1<>'' and img2<>'' ORDER BY title {$order} LIMIT 1;";
			...
			if( ( $imgID < 0 ) || !is_numeric( $imgID )){
				//$nothing = "<p>There are no new picture games to play. <a href=\"index.php?title=Special:PictureGameHome\">Create one!</a></p>";
				$nothing = "<p>" . wfMsgForContent('picturegame_minifeed_nomore') . " <a href=\"index.php?title=Special:PictureGameHome\">" . wfMsgForContent('picturegame_minifeed_nomorecreatelink') . "</a></p>";
				return $nothing;
			}
			...
			$output = "<div class=\"picgame-content\">
					...
					<div class=\"picgame-image-one\">
						...
						<div class=\"picgame-image-one-html\" onmouseout=\"\$El(this).setStyle('background-color', '');\" onmouseover=\"\$El(this).setStyle('background-color', '#4B9AF6');\"><a href=\"index.php?title=Special:PictureGameHome&picGameAction=renderPermalink&id={$imgID}&voteID={$imgID}&voteImage=1&key={$key}\">{$imgOne}</a></div>
					</div>
					<div class=\"picgame-image-two\">
						<div class=\"picgame-image-two-title\">{$img2_caption_text}</div>
						<div class=\"picgame-image-two-html\" onmouseout=\"\$El(this).setStyle('background-color', '');\" onmouseover=\"\$El(this).setStyle('background-color', '#FF0000');\"><a href=\"index.php?title=Special:PictureGameHome&picGameAction=renderPermalink&id={$imgID}&voteID={$imgID}&voteImage=2&key={$key}\">{$imgTwo}</a></div>
					</div>
		...
		function editPanel(){
			...
			$output .= "		</h1>
						<div class=\"submitted-by-image\">
							<a href=\"index.php?title=User:{$row->username}\"><img src=\"{$wgUploadPath}/avatars/{$avatarID}\" style=\"border:1px solid #d7dee8; width:50px; height:50px;\" alt=\"\" border=\"0\"/></a>
						</div>
				...
				<div id=\"complete-buttons\" class=\"complete-buttons\">
					...
					."\"/>
					<input type=\"button\"  onclick=\"window.location='index.php?title=Special:PictureGameHome&picGameAction=renderPermalink&id={$imgID}'\" value=\"".
					//Cancel
		...
		function getImageDivs($isPermalink = false, $imgID = -1, $lastID = -1){
			...
			if($imgID){
				...
				if($next_id){
					...
					if( is_object( $img_one ) ) $preload_thumb = $img_one->getThumbnail( 256, 0, true );
					if( is_object( $preload_thumb ) ) $preload_one_tag = $preload_thumb->toHtml();
					
					$img_two = Image::newFromName( $nextrow->img2 );
					if( is_object( $img_two ) ) $preload_thumb = $img_two->getThumbnail( 256, 0, true );
			...
			if( ( $imgID < 0 ) || !is_numeric( $imgID ) || is_null($row) ){
				...
				$out = "<p>
					...
					<a href=\"index.php?title=Special:PictureGameHome&picGameAction=startCreate\">create your very own</a> or <a href=\"index.php?title=Special:RandomPoll\">take some polls</a>!
				</p>";
				*/
				$out = "<p>" . 
					wfMsgForContent('picturegame_nomore') . "<br>" . wfMsgForContent('picturegame_nomore_2') .
					"<a href=\"index.php?title=Special:PictureGameHome&picGameAction=startCreate\">" . wfMsgForContent('picturegame_nomorecreatelink') . "</a> " . wfMsgForContent('picturegame_nomoreor') . " <a href=\"index.php?title=Special:RandomPoll\">" . wfMsgForContent('picturegame_nomoretakepolls') . "</a>
				</p>";
				$wgOut->addHTML($out);
				return;
			}
			...
			$isShowVotes = false;
			if($lastID > 0){
				$sql = "SELECT * FROM picturegame_images WHERE flag!='FLAGGED' and img1<> '' and img2 <> '' AND id={$lastID};";
				$res = $dbr->query($sql);
				$row = $dbr->fetchObject( $res );
			...
			if($isPermalink || $isShowVotes){
				if( is_object( $img_one ) ) $vote_one_thumb = $img_one->getThumbnail( 40, 0, true );
				if( is_object( $vote_one_thumb ) ) $vote_one_tag = $vote_one_thumb->toHtml();

				if( is_object( $img_two ) ) $vote_two_thumb = $img_two->getThumbnail( 40, 0, true );
			...
			if ($wgUser->isLoggedIn()) {
				...
				<div class=\"create-link\">
					<a href=index.php?title=Special:PictureGameHome&picGameAction=startCreate>
			...
			<div class=\"editDiv\" id=\"editDiv\" style=\"display: none\"> </div>
					...
					<div class=\"imgContent\" id=\"imgContent\">
						...
						<div class=\"pic-game-navigation\">
							<ul>
								<li id=\"backButton\" style=\"display:" . ($lastID > 0 ? "block" : "none") . "\"><a href=\"javascript:window.parent.document.location='index.php?title=Special:PictureGameHome&picGameAction=renderPermalink&id=' + \$('lastid').value\">";
                                                                //Go Back
                                                $output .=       wfMsgForContent('picturegame_backbutton');
                                                $output .=       "</a></li>
								<li id=\"skipButton\" style=\"display:none;\"><a href=\"index.php?title=Special:PictureGameHome&picGameAction=startGame\">";
                                                                //Skip
			...
			$output .=		" </a> -
						<a href=\"javascript:window.parent.document.location='index.php?title=Special:PictureGameHome&picGameAction=renderPermalink&id=' + \$('id').value\"> ";
						//Permalink
		...
		function showHomePage(){
			...
			if( $wgUser->isAllowed('protect') ) {
				...
				$adminlink = "<a href=\"index.php?title=Special:PictureGameHome&picGameAction=adminPanel\"> " . wfMsgForContent('picturegame_adminpanel') . " </a>";
			}
