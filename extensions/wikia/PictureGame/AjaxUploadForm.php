...
class AjaxUploadForm extends UploadForm{
	...
	function mainUploadForm( $msg='' ) {
		...
		//$sourcefilename = 'File';
                $sourcefilename = wfMsgForContent('picturegame_creategamefiletext');
		
		// $summary = wfMsgWikiHtml( 'fileuploadsummary' );
		if($isOverwrite != true)
			//$summary = "Caption";
                        $summary = wfMsgForContent('picturegame_creategamecaptiontext');
		else
			$summary = "";
	...
	<tr>
		<td>
			<input tabindex='9' onClick='submitForm()' type='submit' class='site-button' name='wpUpload' value=\"" .
			//Upload
			wfMsgForContent('picturegame_buttonupload')
			. "\" />
		</td>
	</tr>
	</table>
	...
	function showSuccess() {
		...
		$article->doEdit(  "[[Category:Picturegames]]", "automatic category tagging", EDIT_SUPPRESS_RC);
		}
		$sql = "SELECT img_description FROM  `image` WHERE img_name='". $this->mUploadSaveName ."'";
		$res = $dbr->query($sql);
		$row = $dbr->fetchObject( $res );
