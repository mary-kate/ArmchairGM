...
#determine the DB for the destination site and complain if you can't
      ...
      $dest_dbname = $obj->dest_dbname;
      $wgOut->addHTML("processing request in '$this->mCategory' '$wgDBname' ($dest_dbname)<br>\n");
      if( "" == $dest_dbname ){
	$wgOut->addHTML("Failed to find DB for '$this->mCategory' in the destination list table, assuming $wgDBname<br>\n");
	$dest_db = $wgDBname;
      }

#insert the view's entry into the destination's DB
      $dbw->insert( "`$dest_dbname`.site_view",
...
#		  $wgOut->addHTML("sorry, creation disabled for the moment<br/>");
		...
		$do = wfMsg( 'domain' );
		$ca = wfMsg( 'category' );
		$sc = wfMsg( 'subcategory' );
		$sn = wfMsg( 'sitename' );
		$us = wfMsg( 'username' );
		$c1 = wfMsg( 'color1' );
		$c2 = wfMsg( 'color2' );
		$c3 = wfMsg( 'color3' );
		$bc = wfMsg( 'bordercolor' );
