...
function wfSpecialTopFanBoxes(){
	...
	class TopFanBoxes extends SpecialPage {
		...
		function execute(){
			...
			if($topfanboxcategory == 'User'){
				$wgOut->setPageTitle("Top User Userboxes");
				$categoryfanboxes = $this->getFanBoxByCategory('User');

			}
			elseif($topfanboxcategory == 'Player'){
				//$wgOut->setPageTitle("Top Player Fanboxes");
				//$categoryfanboxes = $this->getFanBoxByCategory('Player');

			}

			elseif($topfanboxid == fantag_date){
			...
			$output .= "<div class=\"fanbox-nav\">
				...
				<p><a href=\"" . $top_title->escapeFullURL("cat=Player") . "\">" . wfMsgForContent("player_fanboxes_link") . "</a><p></div>";
