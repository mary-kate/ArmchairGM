...
class UserActivity {
	...
	private function setEdits(){
		...
		if($this->rel_type)$rel_sql = " where rc_user in (select r_user_id_relation from user_relationship where r_user_id={$this->user_id} and r_type={$this->rel_type}) ";
		if($this->show_current_user)$user_sql = " where rc_user = {$this->user_id}";
	...
	private function setComments(){
		$dbr =& wfGetDB( DB_SLAVE );
		if($this->rel_type)$rel_sql = "and Comment_user_id in (select r_user_id_relation from user_relationship where r_user_id={$this->user_id} and r_type={$this->rel_type})";
		if($this->show_current_user)$user_sql = "and Comment_user_id = {$this->user_id}";
		$sql = "SELECT  UNIX_TIMESTAMP( comment_date) as item_date, Comment_Username, Comment_IP, page_title, vote_count, comment_count, Comment_Text, Comment_user_id, page_namespace, CommentID 
			...
			{$rel_sql} {$user_sql}
			ORDER BY CommentID DESC LIMIT 0," . $this->item_max;
	...
	private function setGiftsRec(){
		...
		$dbr =& wfGetDB( DB_SLAVE );
		if($this->rel_type)$rel_sql = "WHERE ug_user_id_to in (select r_user_id_relation from user_relationship where r_user_id={$this->user_id} and r_type={$this->rel_type} )";
		if($this->show_current_user)$user_sql = "WHERE ug_user_id_to = {$this->user_id}";
	...
	private function setSystemGiftsRec(){
		...
		$dbr =& wfGetDB( DB_SLAVE );
		if($this->rel_type)$rel_sql = "WHERE sg_user_id in (select r_user_id_relation from user_relationship where r_user_id={$this->user_id} and r_type={$this->rel_type} )";
		if($this->show_current_user)$user_sql = "WHERE sg_user_id = {$this->user_id}";
		...
		while ($row = $dbr->fetchObject( $res ) ) {
			...
			$this->items[] = array("id"=>$row->sg_id,"type"=>"system_gift","timestamp"=>$row->item_date ,"pagetitle"=>$row->gift_name,"namespace"=>$row->gift_id,"username"=>$row->sg_user_name,"userid"=>$row->sg_user_id,"comment"=>$row->gift_description,"new"=>"0","minor"=>0 );
		}
	}
	
	private function setRelationships(){
		$dbr =& wfGetDB( DB_SLAVE );
		if($this->rel_type)$rel_sql = "WHERE r_user_id in (select r_user_id_relation from user_relationship where r_user_id={$this->user_id} and r_type={$this->rel_type} )";
		if($this->show_current_user)$user_sql = "WHERE r_user_id = {$this->user_id}";
	...
	private function setMessagesSent(){
		$dbr =& wfGetDB( DB_SLAVE );
		if($this->rel_type)$rel_sql = " ub_user_id_from in (select r_user_id_relation from user_relationship where r_user_id={$this->user_id} and r_type={$this->rel_type} ) and";
		if($this->show_current_user)$user_sql = " ub_user_id_from = {$this->user_id} and";
		...
		while ($row = $dbr->fetchObject( $res ) ) {
			...
			$this->items_grouped[ "user_message" ][ stripslashes($row->ub_user_name) ] ["users"] [ stripslashes($row->ub_user_name_from) ][] = array(
						...
						"pagetitle"=>"","namespace"=>"",
						"username"=>stripslashes($row->ub_user_name_from), "userid"=>$ub_user_id_from, 
	...
	private function setSystemMessages(){
		$dbr =& wfGetDB( DB_SLAVE );
		if($this->rel_type)$rel_sql = " WHERE um_user_id in (select r_user_id_relation from user_relationship where r_user_id={$this->user_id} and r_type={$this->rel_type} )";
		if($this->show_current_user)$user_sql = " WHERE um_user_id = {$this->user_id}";
	...
	function simplifyPageActivity( $type, $has_page = true ){
		...
		if( !is_array( $this->items_grouped[$type] ) ){
			return "";
		}
		foreach( $this->items_grouped[$type] as $page_name => $page_data ){
			...
			foreach( $page_data[ "users" ] as $user_name => $action ){
				...
				if( $has_page && ! $this->displayed[ $type ][$page_name]  ){
				...
				if( $count_users == 1){
					foreach( $this->items_grouped[$type] as $page_name2 => $page_data2 ){
						...
						if( ! $this->displayed[ $type ][$page_name2] &&  count( $page_data2["users"] ) == 1  ) {
			...
			if( $pages || $has_page == false ){
				$this->activityLines[] =  array("type" => $type, "timestamp" => $page_data[ "timestamp" ], $latest_timestamp, "data" => $users . " " . wfMsgExt("useractivity_{$type}", "parsemag", $pages_count, $count_users) . $pages );
			}
		}
	}
