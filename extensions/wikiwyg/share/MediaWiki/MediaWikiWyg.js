...
proto.addModeSelector = function() {
    ...
    for (var i = 0; i < this.wikiwyg.config.modeClasses.length; i++) {
        ...
        var label = Wikiwyg.createElementWithAttrs(
            'label', { 'for': mode_object.classname }
        );
...
proto.format_a = function(element) {
    ...
    label = label.replace(/\s+$/, '');
    /* crappy fix for crappy IE... it's late */
    var href = element.getAttribute('href') ;    
    if (Wikiwyg.is_ie) {	    	    	
    	/* force relative url, grrr
	var server_regexp = new RegExp (wgServer) ;
	if (href.match (server_regexp) ) {
		href.replace (server_regexp, "") ;
	*/
	}
    } else {
	 //   var href = element.getAttribute('href') ;
...
proto.make_wikitext_link = function(label, href, element) {
    ...
    if (! href) {
		...
		/* for IE */
		//href = href.replace (/^http:\/\/.*\/index.php\?title=/i,"?") ;
...
proto.enableThis = function() {
    ...
    /* untested yet, let it await happier times ^_^
    var doc = this.get_edit_document().body ;
    var self = this ;
    var pull_check = function () {
	    self.checkTags () ;
    }

    doc.addEventListener ("click", pull_check, false) ;
    */

}

/* functions for detecting where we are */

proto.checkTags = function () {
	switch (this.getSelectedStyle ('font-weight')) {
		case 'bold' :
      			var bold = document.getElementById ('wikiwyg_button_bold'+this.section_number) ;
			bold.className = 'wikiwyg_button_pressed' ;
			break	
		case 'italic' :
			break
...
proto.getSelectedStyle = function (rule) {
	var strValue = "";
	element = this.getSelectedElement().parentNode ;
	if(document.defaultView && document.defaultView.getComputedStyle){
...
function fixupRelativeUrl(url) {
    ...
    /* run check for addresses alredy contain   */
...
klass.enable_wikiwyg_section = function(section_div, section_number) {
    ...
    edit_button.onclick = function() {
		...
		/* editing this section, user clicked cancel */
...
klass.canSupportWysiwyg = function(div) {
    ...
    return true ;
    try {check_walk(div)} catch(e) { alert(e); return false ; }
    return true;
}
...
proto.editMode = function() {
	...
	if (c) {
		/* get the right object and close it */
...
proto.get_raw_section = function() {
	...
	if (url.match ('/title/') != '') {
		var correctPath = wgArticlePath.replace (/\/\$1/, '') ;
		correctPath = wgArticlePath.replace (/\?title=\$1/, '') ;
		url = wgServer + correctPath + "?title=" + wgPageName + "&action=edit" + "&section=" + this.section_number ;
	} else {
...
proto.submit_action_form = function(action, value) {
    ...
    var callback = function() {
      ...
      //      document.body.removeChild(tempIFrame);            
	    /* omitting CrappyHack - this way it's faster *and* more reliable */
          self.cancelEdit () ;
...
proto.make_button = function(type, label) {
    ...
    return Wikiwyg.createElementWithAttrs(
        'img', {
            ...
            alt: label,
            title: label,
	    id: 'wikiwyg_button_' + type + this.section_number ,
            src: base + type + ext
        }
    );
}
