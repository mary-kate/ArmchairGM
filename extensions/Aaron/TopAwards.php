...
class TopAwards extends SpecialPage {
	...
	function execute(){
		...
		$categories = array (
				array ("category_name"=>"Edit", "category_threshold"=>"0", "category_id"=>1),
				array ("category_name"=>"Vote", "category_threshold"=>"0", "category_id"=>2),
				array ("category_name"=>"Comment", "category_threshold"=>"0", "category_id"=>3),
				array ("category_name"=>"Recruit", "category_threshold"=>"0", "category_id"=>7),
				array ("category_name"=>"Friend", "category_threshold"=>"0", "category_id"=>8)
			);
		...
		//page title 
		$wgOut->setPageTitle("{$page_category} Awards");
		...
		$output .= "<div class=\"top-awards-navigation\">
			...
			foreach($categories as $award_type) {
				...
				if ($nav_x == $category_number) {
					$output .= "<p><b>{$award_type[category_name]}</b></p>";
				} else {
					$output .= "<p><a href=\"index.php?title=Special:TopAwards&category={$nav_x}\">{$award_type[category_name]}</a></p>";
				}	
				$nav_x++;
			}
		...
		while ($row = $dbr->fetchObject($res)) {
			...
			$gift_name = "{$top_gift} {$categories[$category_number][category_name]}".(($top_gift>1)?"s":"")." Milestone";
