...
$wgExtensionFunctions[] = 'wfMLBTeamLeadersBattingAvg';

function wfMLBTeamLeadersBattingAvg(){
  global $wgUser,$IP;
  include_once("includes/SpecialPage.php");


class MLBTeamLeadersBattingAvg extends SpecialPage {

	
	function MLBTeamLeadersBattingAvg(){
		UnlistedSpecialPage::UnlistedSpecialPage("MLBTeamLeadersBattingAvg");
	}
	...
	function execute(){
		...
		foreach ($batting_cat_text as $stat=>$stat_title) {
			...
			foreach ($teams as $team_name=>$team_query) {
				...
				if($stat=="AVG") {
					$sql="SELECT * FROM (SELECT mlb_batting_stats.player_id, ROUND((SUM(H)+SUM(BB)+SUM(HBP))/SUM(AB),3) as statistic, SUM(AB)+SUM(BB)+SUM(HBP)+SUM(SF) as plate_appearance, player_name FROM mlb_batting_stats JOIN mlb_players_info ON mlb_batting_stats.player_id=mlb_players_info.player_id WHERE ($team_query) GROUP BY player_id, player_name) as data WHERE plate_appearance>2000 ORDER BY statistic DESC LIMIT 0,25";
				} else if ($stat=="OBP") {
					$sql="SELECT * FROM (SELECT mlb_batting_stats.player_id, ROUND((SUM(H)+SUM(BB)+SUM(HBP))/(SUM(AB)+SUM(BB)+SUM(HBP)+SUM(SF)),3) as statistic, SUM(AB)+SUM(BB)+SUM(HBP)+SUM(SF) as plate_appearance, player_name FROM mlb_batting_stats JOIN mlb_players_info ON mlb_batting_stats.player_id=mlb_players_info.player_id WHERE ($team_query) GROUP BY player_id, player_name) as data WHERE plate_appearance>2000 ORDER BY statistic DESC LIMIT 0,25";
				} else if ($stat=="SLG") {
					$sql="SELECT * FROM (SELECT mlb_batting_stats.player_id, ROUND((SUM(H)+SUM(2B)+(2*SUM(3B))+(3*SUM(HR)))/SUM(AB),3) as statistic, SUM(AB)+SUM(BB)+SUM(HBP)+SUM(SF) as plate_appearance, player_name FROM mlb_batting_stats JOIN mlb_players_info ON mlb_batting_stats.player_id=mlb_players_info.player_id WHERE ($team_query) GROUP BY player_id, player_name) as data WHERE plate_appearance>2000 ORDER BY statistic DESC LIMIT 0,25";
				} else if ($stat=="OPS") {
					$sql="SELECT * FROM (SELECT mlb_batting_stats.player_id, ROUND((((SUM(H)+SUM(BB)+SUM(HBP))/(SUM(AB)+SUM(BB)+SUM(HBP)+SUM(SF)))+((SUM(H)+SUM(2B)+(2*SUM(3B))+(3*SUM(HR)))/SUM(AB))),3) as statistic, SUM(AB)+SUM(BB)+SUM(HBP)+SUM(SF) as plate_appearance, player_name FROM mlb_batting_stats JOIN mlb_players_info ON mlb_batting_stats.player_id=mlb_players_info.player_id WHERE ($team_query) GROUP BY player_id, player_name) as data WHERE plate_appearance>2000 ORDER BY statistic DESC LIMIT 0,25";
				}
				...
				$title_string .= "List of {$team_name} players with the highest {$stat_title}";
...
SpecialPage::addPage( new MLBTeamLeadersBattingAvg );
