...
$wgExtensionFunctions[] = 'wfMLBTopCategoryPages';

function wfMLBTopCategoryPages(){
  global $wgUser,$IP;
  include_once("includes/SpecialPage.php");


class MLBTopCategoryPages extends SpecialPage {

	
	function MLBTopCategoryPages(){
		UnlistedSpecialPage::UnlistedSpecialPage("MLBTopCategoryPages");
	}
	...
	function execute(){
		...
		$sql_year = "SELECT DISTINCT year FROM mlb_pitching_stats WHERE year ORDER BY year ASC";
		...
		foreach ($pitch_cat_text as $stat=>$stat_title) {
				...
				$sql = "SELECT DISTINCT player_name, {$stat}, player_id FROM (SELECT mlb_players_info.player_id, mlb_players_info.player_name, Year, {$stat} FROM (SELECT mlb_pitching_stats.* FROM mlb_pitching_stats UNION SELECT mlb_pitching_stats_total.* FROM mlb_pitching_stats_total WHERE Year <> 9999) as all_pitching JOIN mlb_players_info ON mlb_players_info.player_id=all_pitching.player_id ORDER BY {$stat} DESC, player_name ASC) AS the_counts WHERE Year={$baseball_year} and {$stat}>0 LIMIT 0,25";
				$res = $dbr->query($sql);
				...
				$title_string .= "List of pitchers with the most {$stat_title} during the {$baseball_year} baseball season";
				...
				//$article_text .= "[[Category:".ucfirst($stat_title)." Lists]]\n";
...
SpecialPage::addPage( new MLBTopCategoryPages );
