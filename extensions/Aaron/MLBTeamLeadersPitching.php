...
$wgExtensionFunctions[] = 'wfMLBTeamLeadersPitching';

function wfMLBTeamLeadersPitching(){
  global $wgUser,$IP;
  include_once("includes/SpecialPage.php");


class MLBTeamLeadersPitching extends SpecialPage {

	
	function MLBTeamLeadersPitching(){
		UnlistedSpecialPage::UnlistedSpecialPage("MLBTeamLeadersPitching");
	}
	...
	function execute(){
		...
		foreach ($pitching_cat_text as $stat=>$stat_title) {
		
			foreach ($teams as $team_name=>$team_query) {
			
				$dbr =& wfGetDB( DB_MASTER );
				
				$sql = "SELECT * FROM (SELECT mlb_pitching_stats.player_id, sum({$stat}) as statistic, player_name FROM mlb_pitching_stats JOIN mlb_players_info ON mlb_pitching_stats.player_id=mlb_players_info.player_id WHERE ($team_query) GROUP BY player_id, player_name) as data ORDER BY statistic DESC LIMIT 0,25";
				$res = $dbr->query($sql);
				...
				$title_string .= "List of {$team_name} pitchers with the most {$stat_title}";
...
SpecialPage::addPage( new MLBTeamLeadersPitching );
