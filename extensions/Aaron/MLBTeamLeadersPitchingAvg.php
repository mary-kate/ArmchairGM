...
$wgExtensionFunctions[] = 'wfMLBTeamLeadersPitchingAvg';

function wfMLBTeamLeadersPitchingAvg(){
  global $wgUser,$IP;
  include_once("includes/SpecialPage.php");


class MLBTeamLeadersPitchingAvg extends SpecialPage {

	
	function MLBTeamLeadersPitchingAvg(){
		UnlistedSpecialPage::UnlistedSpecialPage("MLBTeamLeadersPitchingAvg");
	}
	...
	function execute(){
		...
		foreach ($batting_cat_text as $stat=>$stat_title) {
			...
			foreach ($teams as $team_name=>$team_query) {
				...
				if ($stat=="ERA") {	
					$sql="SELECT * FROM (SELECT mlb_pitching_stats.player_id, ROUND((9*SUM(ER))/SUM(IP),3) as statistic, SUM(IP) as IP, player_name FROM mlb_pitching_stats JOIN mlb_players_info ON mlb_pitching_stats.player_id=mlb_players_info.player_id WHERE ({$team_query}) GROUP BY player_id, player_name) as data WHERE IP>500 ORDER BY statistic ASC LIMIT 0,25";
				} else if ($stat=="WHIP") {	
					$sql="SELECT * FROM (SELECT mlb_pitching_stats.player_id, ROUND((SUM(BB)+SUM(H))/SUM(IP),2) as statistic, SUM(IP) as IP, player_name FROM mlb_pitching_stats JOIN mlb_players_info ON mlb_pitching_stats.player_id=mlb_players_info.player_id WHERE ({$team_query}) GROUP BY player_id, player_name) as data WHERE IP>500 ORDER BY statistic ASC LIMIT 0,25";
				} else if ($stat=="Winning Percentage") {
					$sql="SELECT * FROM (SELECT mlb_pitching_stats.player_id, ROUND(SUM(W)/(SUM(W)+SUM(L)),3) as statistic, SUM(W)+SUM(L) as decisions, player_name FROM mlb_pitching_stats JOIN mlb_players_info ON mlb_pitching_stats.player_id=mlb_players_info.player_id WHERE ({$team_query}) GROUP BY player_id, player_name) as data WHERE decisions>75 ORDER BY statistic DESC LIMIT 0,25";
				}
				...
				if ($stat=="Winning Percentage") {
					$title_string .= "List of {$team_name} pitching with the highest {$stat_title}";
				} else {
					$title_string .= "List of {$team_name} pitching with the lowest {$stat_title}";
				}
				...
				$categories = array( "Lists", "Statistical Lists", "Baseball Lists","Pitching Lists","MLB Lists", "{$team_name} Lists", "{$stat} Lists");
...
SpecialPage::addPage( new MLBTeamLeadersPitchingAvg );
