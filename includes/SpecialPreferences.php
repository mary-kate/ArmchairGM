...
class PreferencesForm {
	...
	var $mSearch, $mRecent, $mHourDiff, $mSearchType, $mSearchLines, $mSearchChars, $mAction;
	...
	function mainPrefsForm( $status , $message = '' ) {
		...
		global $wgContLanguageCode, $wgDefaultSkin, $wgSkipSkins, $wgAuth, $wgDisableInternalSearch;
		...
		$wgOut->addHTML( '<fieldset><legend>' . wfMsg( 'textboxsize' ) . '</legend>
			...
			"</div>" .
			$this->getToggles( $this->getEditingTab ()
			) . '</fieldset>'
		);
		...
		# Search
		if ($wgDisableInternalSearch == true) { //wikia search
			$wgOut->addHTML( '<fieldset><legend>' . wfMsg( 'searchresultshead' ) . '</legend>' );
			$wgOut->addHTML( '<table>' );
			$wgOut->addHTML(
				$this->addRow(
			...
			$wgOut->addHTML( '</table><fieldset><legend>' . wfMsg('searchtype') . '</legend>' );
			...
			foreach ($searchRadio as $key => $val ) {
				...
				$wgOut->addHTML( "<input type='radio' name='wpSearchType' id=\"$key\" value=\"$key\"$checked /> <label for=\"mate$key\">{$val}</label><br />\n" );
			}

			$wgOut->addHTML( '</fieldset></fieldset>' );
