	...
	function writeRevision( $row ) {
		...
		if( $row->rev_deleted & Revision::DELETED_TEXT ) {
			...
			if ( $wgAddFromLink && ($row->page_namespace != 8) && ($row->page_namespace != 10) ) {
				...
				}else if (isset($wgSharedDB)){
					global $wgServer,$wgArticlePath,$wgSitename;
					$DB =& wfGetDB( DB_SLAVE );
					$_row = $DB->selectRow( '`'.$wgSharedDB.'`.`interwiki`', array( 'iw_prefix' ), array( 'iw_url' => $wgServer.$wgArticlePath ) );
					if ($_row) {
						$text .= '<div id="wikia-credits"><br /><br /><small>' . wfMsg('tagline-url-interwiki',$_row->iw_prefix) . '</small></div>';
					}
					else {
