...
class OutputPage {
	...
	function checkLastModified ( $timestamp ) {
		...
		//echo "<timest tm='{$timestamp}'></timest>";
		...
		if( !$this->mEnableClientCache) $timestamp =  wfTimeStamp(TS_MW);
		$lastmod = wfTimestamp( TS_RFC2822, max( $timestamp, $wgUser->mTouched, $wgCacheEpoch ) );

		//echo "test:".$_SERVER['HTTP_IF_MODIFIED_SINCE'];
	...
	public function sendCacheControl() {
		...
		//echo "testing:" . $this->mEnableClientCache . "<BR>";
		...
		if( !$this->uncacheableBecauseRequestvars() && $this->mEnableClientCache ) {
			...
			if(session_id() == '') {
			//                $wgRequest->response()->header( 'Expires: ' . gmdate( 'D, d M Y H:i:s', time() + 30 ) . ' GMT' );
			    if($this->mLastModified) $wgRequest->response()->header( "Last-modified: {$this->mLastModified}" );
	...
	public function returnToMain( $auto = true, $returnto = NULL ) {
		...
		$sk = $wgUser->getSkin();
		#FIXME
		if ( is_string( $returnto ) && ( strpos( $returnto, "/forum" ) === 0 || strpos( $returnto, "/phpbb" ) === 0 ) ) {
			$r = wfMsg( 'returnto', "<a href='$returnto'>Forum</a>" );
			if ( $auto ) {
				$wgOut->addMeta( 'http:Refresh', '10;url=' . $returnto );
			}
		} else {
	...
	public function headElement() {
		...
		wfDebug("head element {$this->mScripts}\n");
		$ret .= "<!DOCTYPE html PUBLIC \"$wgDocType\"\n        \"$wgDTD\">\n";
		//echo "ouputting:" . $this->getHTMLTitle() . "<BR>";
		...
		$printsheet = htmlspecialchars( "http://images.wikia.com/common/wikiany/css/wikiprintable.css?$wgStyleVersion" );
