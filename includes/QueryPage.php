...
class QueryPage {
	 ...
	 */
	function doFeed( $class = '', $limit = 50, $raw = false ) {
		...
		if( isset($wgFeedClasses[$class]) ) {
			...
			while( $obj = $dbr->fetchObject( $res ) ) {
				...
				if( !$raw ) {
				    ...
				    $parsedFeed = $wgParser->parse( $item->getDescription(), $wgTitle, new ParserOptions(), false );
				    $item->Description = $parsedFeed->mText;
				}
