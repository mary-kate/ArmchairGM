...
class ApiMain extends ApiBase {
	...
	*/
	public function __construct($request, $enableWrite = false) {
		...
		if (is_array( $wgApiMainListFormats )) {
			foreach ( $wgApiMainListFormats as $lformat => $aformat) {
				$this->mFormats[$lformat] = $aformat;
			}
		}
		
		global $wgApiMainListModules;
		if (is_array( $wgApiMainListModules )) {
			foreach ( $wgApiMainListModules as $lmodule => $amodule) {
				$this->mModules[$lmodule] = $amodule;
			}
		}
